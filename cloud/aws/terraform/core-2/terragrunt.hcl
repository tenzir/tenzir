...
inputs = {
  ...
  vast_lambda_image        = run_cmd("bash", "-c", "VASTCLOUD_NOTTY=1 ../../vast-cloud current-image --repo-arn ${dependency.step_1.outputs.vast_lambda_repository_arn}")
  vast_server_image        = run_cmd("bash", "-c", "VASTCLOUD_NOTTY=1 ../../vast-cloud current-image --repo-arn ${dependency.step_1.outputs.vast_fargate_repository_arn}")
