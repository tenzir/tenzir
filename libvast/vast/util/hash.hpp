#ifndef VAST_UTIL_HASH_HPP
#define VAST_UTIL_HASH_HPP
...
struct hash {
  ...
  template <typename T>
  auto add(T const& x) -> std::enable_if_t<is_hashable<T>::value, bool> {
  ...
  template <typename Hack = Derived>
  auto get() -> decltype(std::declval<Hack>().compute()) {
    static_assert(std::is_same<Hack, Derived>::value, ":-P");
