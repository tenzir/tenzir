...
std::enable_if_t<I == sizeof...(Ts)> static_for_each(F, std::tuple<Ts...>&) {
}

template <size_t I = 0, typename F, typename... Ts>
std::enable_if_t<I == sizeof...(Ts)> static_for_each(F,
                                                     std::tuple<Ts...> const&) {
}
...
template <size_t I = 0, typename F, typename... Ts>
  std::enable_if_t
  < I<sizeof...(Ts)> static_for_each(F f, std::tuple<Ts...>& t) {
  f(std::get<I>(t));
...
template <size_t I = 0, typename F, typename... Ts>
  std::enable_if_t
  < I<sizeof...(Ts)> static_for_each(F f, std::tuple<Ts...> const& t) {
  f(std::get<I>(t));
  static_for_each<I + 1, F, Ts...>(f, t);
}
