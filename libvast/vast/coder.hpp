...
#include "vast/detail/range_eval_opt.hpp"
...
class vector_coder
  : util::equality_comparable<vector_coder<Derived, Bitstream>> {
  friend access;

public:
  using bitstream_type = Bitstream;

  vector_coder(size_t cardinality = 0) : bitstreams_(cardinality) {
  }
...
private:
  ...
  template <typename C, typename T>
  auto decode(coder_array<C> const& coders, relational_operator op, T x) const
    -> std::enable_if_t<is_equality_coder<C>{} || is_bitslice_coder<C>{},
