#ifndef VAST_CONCEPT_SERIALIZABLE_DESERIALIZER_HPP
#define VAST_CONCEPT_SERIALIZABLE_DESERIALIZER_HPP
...
public:
  ...
  template <typename T>
  auto get(T& x) -> decltype(deserialize(std::declval<Derived&>(), x, 0)) {
  ...
  template <typename T>
  auto get(T& x) -> decltype(deserialize(std::declval<Derived&>(), x)) {
