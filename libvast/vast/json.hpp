...
public:
  ...
  using types
    = caf::detail::type_list<null, boolean, number, string, array, object>;
  ...
  template <typename T, typename
                        = std::enable_if_t<std::is_constructible_v<json, T>>>
...
template <class T, int = std::is_convertible_v<
                           T, json::number> && !std::is_same_v<T, json::boolean>
                           ? 1
