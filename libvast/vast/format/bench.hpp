...
/// A real measuring  benchmark mixin based on CPU cycles.
...
public:
  ~cycleclock_benchmark_mixin() {
    ...
    for (auto i = 0; i < N; ++i) {
      std::cout << "step_" << i << ": " << events_per_step[i] << " events in "
...
public:
  ~timespec_benchmark_mixin() {
    std::cout << "timespec benchmark results\n";
    for (auto i = 0; i < N; ++i) {
      std::cout << "step_" << i << ": " << events_per_step[i] << " events in "
                << events_durations[i] << " nsec";

      if (events_per_step[i] > 0) {
        const auto cycles_on_event
          = static_cast<double>(events_durations[i]) / events_per_step[i];
        std::cout << " => " << cycles_on_event << " nsec/event";
      }
      std::cout << "\n";
    }
    std::cout << std::endl;
