...
template <>
struct formatter<enum vast::query_processor::state_name> {
  ...
  auto format(enum vast::query_processor::state_name value,
              FormatContext& ctx) const {
    switch (value) {
      case vast::query_processor::state_name::idle:
        return format_to(ctx.out(), "idle");
      case vast::query_processor::state_name::await_query_id:
        return format_to(ctx.out(), "await_query_id");
      case vast::query_processor::state_name::await_results_until_done:
