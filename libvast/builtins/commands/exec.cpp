...
auto exec_command(const invocation& inv, caf::actor_system& sys)
  ...
  if (dump_error) {
    ...
    auto diag = make_diagnostic_printer(filename, content, true, stream);
    ...
    // Create a copy with all colors removed.
    auto without_colors = std::string{};
    for (auto it = result.begin(); it != result.end(); ++it) {
      if (*it == '\033' && it + 1 != result.end() && *(it + 1) == '[') {
        while (it != result.end() && *it != 'm') {
          ++it;
        }
      } else {
  ...
  if (not parsed) {
    ...
    // TODO: Find a better way to exit without printing anything else.
    std::exit(1);
  }
