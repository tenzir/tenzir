...
public:
  ...
  auto parse_parser(tql::parser_interface& p) const
    ...
    auto parser = tql::argument_parser{"xsv", "https://vast.io/docs/understand/"
                                              "formats/xsv"};
...
public:
  ...
  auto parse_printer(tql::parser_interface& p) const
    -> std::unique_ptr<plugin_printer> override {
    tql::argument_parser{name()}.parse(p);
    return std::make_unique<xsv_printer>(xsv_printer::args{
      .field_sep{Sep}, .list_sep{ListSep}, .null_value{Null.str()}});
  }
