...
auto inspect_with_plugin(Inspector& f, std::unique_ptr<T>& x) -> bool {
  if constexpr (Inspector::is_loading) {
    auto name = std::string{};
    if (!f.apply(name)) {
      return false;
    }
    auto const* p = plugins::find<serialization_plugin<T>>(name);
    if (!p) {
...
public:
  ...
  auto operator()(operator_control_plane& ctrl) const
    ...
    return caf::make_error(ec::unspecified,
                           "TODO: could not instantiate loader");
  }
  ...
  friend auto inspect(auto& f, load_operator& x) -> bool {
    return inspect_with_plugin(f, x.loader_);
  }
...
public:
  ...
  friend auto inspect(auto& f, parse_operator& x) -> bool {
    return inspect_with_plugin(f, x.parser_);
  }
  ...
  operator()(generator<chunk_ptr> input, operator_control_plane& ctrl) const
    ...
    if (not parser) {
      return caf::make_error(ec::unspecified,
                             "TODO: could not instantiate parser");
    }
...
[[noreturn]] void throw_loader_not_found(const located<std::string>& x) {
  ...
  diagnostic::error("loader `{}` could not be found", x.inner)
    .primary(x.source)
    .note("must be one of {}", fmt::join(available, ", "))
    .docs("https://vast.io/docs/understand/connectors")
    .throw_();
}
...
[[noreturn]] void throw_parser_not_found(const located<std::string>& x) {
  ...
  diagnostic::error("parser `{}` could not be found", x.inner)
    .primary(x.source)
    .note("must be one of {}", fmt::join(available, ", "))
    .docs("https://vast.io/docs/understand/formats")
    .throw_();
}
...
public:
  ...
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs = "https://vast.io/docs/understand/operators/sources/from";
    ...
    auto q = tql::until_keyword_parser{"read", p};
...
public:
  ...
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs = "https://vast.io/docs/understand/operators/sources/read";
    auto p_name = p.accept_shell_arg();
    if (!p_name) {
      diagnostic::error("expected parser name")
        .primary(p.current_span())
    ...
    auto q = tql::until_keyword_parser{"from", p};
...
public:
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs = "https://vast.io/docs/understand/operators/sources/load";
...
public:
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs
      = "https://vast.io/docs/understand/operators/transformations/parse";
    auto name = p.accept_shell_arg();
    if (!name) {
      diagnostic::error("expected parser name")
        .primary(p.current_span())
