...
public:
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs
      = "https://vast.io/docs/understand/operators/transformations/print";
...
public:
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs = "https://vast.io/docs/understand/operators/sinks/save";
    auto name = p.accept_shell_arg();
    if (!name) {
...
public:
  ...
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs = "https://vast.io/docs/understand/operators/sinks/write";
    ...
    auto q = tql::until_keyword_parser{"to", p};
    ...
    // It could be that `printer->allows_joining()` is false, but
    // `saver->is_joining()` is true. This can cause conflicts later,
...
public:
  ...
  auto parse_operator(tql::parser_interface& p) const -> operator_ptr override {
    ...
    auto docs = "https://vast.io/docs/understand/operators/sinks/to";
    auto l_name = p.accept_shell_arg();
    if (!l_name) {
      diagnostic::error("expected saver name")
        .primary(p.current_span())
    ...
    auto q = tql::until_keyword_parser{"write", p};
    ...
    // TODO: Check this (also for `write`).
    // It could be that `printer->allows_joining()` is false, but
    // `saver->is_joining()` is true. This can cause conflicts later,
    // but `print` contains the neccesary checks and would abort the execution.
    if (not saver->is_joining()) {
