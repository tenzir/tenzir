...
struct drop_step_configuration {
  /// The key suffixes of the fields to drop.
  std::vector<std::string> fields = {};

  /// Support type inspection for easy parsing with convertible.
  template <class Inspector>
  friend auto inspect(Inspector& f, drop_step_configuration& x) {
    return f(x.fields);
  }
...
public:
  explicit drop_step(drop_step_configuration config) noexcept
...
private:
  ...
  drop_step_configuration config_;
};

class drop_step_plugin final : public virtual transform_plugin {
public:
  // plugin API
  ...
  make_transform_step(const record& options) const override {
    if (!options.contains("fields"))
      return caf::make_error(ec::invalid_configuration,
                             ...
                             "drop step");
    auto config = to<drop_step_configuration>(options);
    if (!config)
      return config.error();
...
VAST_REGISTER_PLUGIN(vast::drop_step_plugin)
