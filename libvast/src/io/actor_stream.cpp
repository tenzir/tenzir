...
#include "vast/io/actor_stream.hpp"
...
actor_input_stream::actor_input_stream(actor source,
  ...
  : source_{std::move(source)}, timeout_{timeout} {
...
actor_output_device::actor_output_device(actor sink) : sink_{std::move(sink)} {
}
...
actor_output_stream::actor_output_stream(actor sink, size_t block_size)
  : device_{std::move(sink)}, buffered_stream_{device_, block_size} {
}
