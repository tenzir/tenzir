...
block::writer::writer(block& blk)
  : block_{blk},
    ...
    compressed_stream_{
      make_compressed_output_stream(block_.compression_, base_stream_)},
...
block::reader::reader(block const& blk)
  : block_{blk},
    ...
    compressed_stream_{
      make_compressed_input_stream(block_.compression_, base_stream_)},
...
block::block(io::compression method) : compression_(method) {
}
...
bool operator==(block const& x, block const& y) {
  return x.compression_ == y.compression_ && x.elements_ == y.elements_
