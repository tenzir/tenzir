...
int compressedbuf::sync() {
  ...
  // Write header.
  char size[16];
  auto n = detail::varbyte::encode(uncompressed_size, size);
  size_t put = streambuf_.sputn(size, n);
  if (put != n)
    return -1;
  total += put;
  n = detail::varbyte::encode(compressed_.size(), size);
...
compressedbuf::int_type compressedbuf::underflow() {
  ...
  detail::varbyte::decode(uncompressed_size, size);
  if (!read_varbyte(streambuf_, size))
    return traits_type::eof();
  detail::varbyte::decode(compressed_size, size);
  // Adjust buffers.
  uncompressed_.resize(uncompressed_size);
