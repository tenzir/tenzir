...
auto parse_endpoint_parameters(const vast::rest_endpoint& endpoint, const detail::stable_map<std::string, std::string>& params) -> caf::expected<vast::record> {
  vast::record result;
  if (!endpoint.params)
    return result;
  for (auto const& leaf : endpoint.params->leaves()) {
    ...
    auto typed_value
      = caf::visit(
        ...
        leaf.field.type);
    if (!typed_value)
      return caf::make_error(ec::invalid_argument, fmt::format("failed to parse parameter "
                             "'{}' with value '{}': {}\n",
