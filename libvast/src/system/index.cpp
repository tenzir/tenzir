...
caf::error index_state::extract_partition_synopsis(
  const vast::path& partition_path, const vast::path& partition_synopsis_path) {
...
caf::error index_state::load_from_disk() {
  ...
  if (auto fname = index_filename(); exists(fname)) {
    ...
    for (auto uuid_fb : *partition_uuids) {
      ...
      // (FIXME: create separate function)
...
index(index_actor::stateful_pointer<index_state> self,
  ...
  VAST_TRACE(VAST_ARG(filesystem), VAST_ARG(dir), VAST_ARG(partition_capacity),
             ...
             VAST_ARG(num_workers), VAST_ARG(meta_index_dir),
      ...
      self->request(self->state.filesystem, caf::infinite, atom::mmap_v, path)
        .then(
          [=](chunk_ptr chunk) mutable {
            ...
            if (!rm(synopsis_path))
              VAST_WARNING(self, "could not unlink partition synopsis at",
                           path);
