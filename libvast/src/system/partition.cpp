...
bool partition_selector::operator()(const vast::qualified_record_field& filter,
  ...
  vast::qualified_record_field fqf{layout.name(), layout.fields.at(x.index())};
  return filter == fqf;
}
...
active_partition(caf::stateful_actor<active_partition_state>* self, uuid id,
  ...
  // The active partition stage is a caf stream stage that takes a stream of
  // `table_slice` as input and produces several streams of `table_slice_column`
  // as output.
