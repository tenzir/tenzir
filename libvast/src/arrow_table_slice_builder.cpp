...
make_column_builder(const type& t, arrow::MemoryPool* pool);
...
// -- schema/type transformation helpers definitions ---------------------------
...
std::shared_ptr<arrow::Schema> make_arrow_schema(const record_type& t) {
  ...
  for (auto& field : t.fields) {
    ...
    fields.emplace_back(std::move(field_ptr));
  }
...
arrow_table_slice_builder::arrow_table_slice_builder(record_type layout) noexcept
...
arrow_table_slice_builder::~arrow_table_slice_builder() noexcept = default;

table_slice::size_type arrow_table_slice_builder::rows() const noexcept {
  return rows_;
}

table_slice_encoding arrow_table_slice_builder::encoding() const noexcept {
#if VAST_HAVE_ARROW
...
void arrow_table_slice_builder::reserve(table_slice::size_type) {
  // nop
}

bool arrow_table_slice_builder::add_impl(data_view x) {
#if VAST_HAVE_ARROW
  if (!builders_[column_]->add(std::move(x)))
    return false;
...
caf::expected<chunk_ptr> arrow_table_slice_builder::finish_impl() {
#if VAST_HAVE_ARROW
  // Create a `vast.fbs.table_slice.arrow.v0` table slice.
  auto builder = fbs::table_slice::arrow::v0Builder{fbb_};
  ...
  // Build record batch.
  auto schema = std::shared_ptr<arrow::Schema>{};
  auto columns = std::vector<std::shared_ptr<arrow::Array>>{};
  columns.reserve(builders_.size());
  for (auto& builder : builders_)
    columns.emplace_back(builder->finish());
  auto record_batch = arrow::RecordBatch::Make(schema, rows_, columns);
  auto buffer = arrow::ipc::SerializeRecordBatch(
                  *record_batch, arrow::ipc::IpcWriteOptions::Defaults())
                  .ValueOr({});
  if (!buffer)
    return make_error(ec::format_error, "failed to write Arrow Buffer");
  builder.add_record_batch(fbb_.CreateVector(buffer->data(), buffer->size()));
  // Build the table slice.
  auto encoded_slice = builder.Finish();
  fbs::CreateTableSlice(fbb_, fbs::table_slice::TableSlice::arrow_v0,
                        encoded_slice.Union());
#else
...
void arrow_table_slice_builder::reset_impl() {
  // nop
}
