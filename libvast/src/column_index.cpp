...
make_column_index(caf::actor_system& sys, path filename, type column_type,
  ...
  auto result = std::make_unique<column_index>(
    sys, std::move(column_type), std::move(index_opts), std::move(filename));
...
void column_index::add(const table_slice_column& x) {
  VAST_TRACE(VAST_ARG(x));
  if (has_skip_attribute_)
    return;
  x.slice->append_column_to_index(x.column, *idx_);
}
