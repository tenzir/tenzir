...
#include "vast/partition_transformer.hpp"
...
TEST(pass pipeline / done before persist) {
  ...
  auto store_id = std::string{vast::defaults::store_backend};
  auto synopsis_opts = vast::index_config{};
  auto index_opts = caf::settings{};
  auto transformer
    = self->spawn(vast::partition_transformer, store_id, synopsis_opts,
...
TEST(delete pipeline / persist before done) {
  ...
  auto transformer
    = self->spawn(vast::partition_transformer, store_id, synopsis_opts,
                  ...
                  unbox(vast::pipeline::internal_parse("drop uid")),
                  PARTITION_PATH_TEMPLATE, SYNOPSIS_PATH_TEMPLATE);
  REQUIRE(transformer);
  // Stream data
  auto rp = self->request(transformer, caf::infinite, vast::atom::persist_v);
...
TEST(pass partition pipeline via the index) {
  ...
  auto catalog = self->spawn(vast::catalog, accountant, directory / "types");
  ...
  auto index
    = self->spawn(vast::index, accountant, filesystem, catalog, index_dir,
                  vast::defaults::store_backend, partition_capacity,
  ...
  auto rp3
    = self->request(index, caf::infinite, vast::atom::apply_v, vast::pipeline{},
                    partition_infos, vast::keep_original_partition::yes);
  run();
  rp3.receive(
  ...
  auto rp5
    = self->request(index, caf::infinite, vast::atom::apply_v, vast::pipeline{},
                    partition_infos, vast::keep_original_partition::no);
  run();
  rp5.receive(
    [=](const std::vector<vast::partition_info>& infos) {
      REQUIRE_EQUAL(infos.size(), 1ull);
...
TEST(query after transform) {
  ...
  auto index_dir = std::filesystem::path{"/vast/index"};
  auto catalog = self->spawn(vast::catalog, accountant, directory / "types");
  const auto partition_capacity = vast::defaults::max_partition_size;
  const auto active_partition_timeout = vast::duration{};
  const auto in_mem_partitions = 10;
  const auto taste_count = 1;
  const auto num_query_supervisors = 10;
  ...
  run();
  rp1.receive(
    [&](vast::catalog_lookup_result crs) {
  ...
  auto rp3 = self->request(index, caf::infinite, vast::atom::apply_v,
                           unbox(vast::pipeline::internal_parse(
  ...
  auto count_results = [&](const std::string& query_string) -> size_t {
    ...
    run();
    rp4.receive(
      [&](const vast::query_cursor& cursor) {
...
TEST(select pipeline with an empty result set) {
  ...
  auto rp2
    = self->request(index, caf::infinite, vast::atom::apply_v,
                    unbox(vast::pipeline::internal_parse("where #schema == "
                                                         "\"does_not_exist\"")),
                    partition_infos, vast::keep_original_partition::no);
  run();
  rp2.receive(
