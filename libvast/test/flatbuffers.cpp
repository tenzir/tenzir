...
TEST(index roundtrip) {
  ...
  state.stats.layouts["zeek.conn"]
    = vast::system::v2::layout_statistics{54931u};
  // Serialize the index.
...
TEST(full partition roundtrip) {
  ...
  auto partition
    = sys.spawn(vast::system::partition, partition_uuid, fs, caf::settings{});
  run();
  REQUIRE(partition);
  ...
  auto readonly_partition
    = sys.spawn(vast::system::readonly_partition, partition_uuid, *chunk);
