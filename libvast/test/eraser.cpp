...
auto mock_index(index_actor::stateful_pointer<mock_index_state>)
  ...
  return {
    ...
    [=](atom::subscribe, atom::create, vast::partition_creation_listener_actor,
    ...
    [=](atom::resolve, vast::expression) -> catalog_lookup_result {
    ...
    [=](atom::evaluate, vast::query_context&) -> caf::result<query_cursor> {
      FAIL("no mock implementation available");
    },
...
struct fixture : fixtures::deterministic_actor_system_and_events {
  ...
  void spawn_aut(std::string query = ":timestamp < 1 week ago") {
    ...
    aut = sys.spawn(vast::eraser, 500ms, std::move(query), index);
    sched.run();
  }
...
TEST(eraser on mock INDEX) {
  ...
  expect((vast::catalog_lookup_result), from(index).to(aut));
  expect((atom::apply, vast::pipeline, std::vector<vast::partition_info>,
          vast::keep_original_partition),
         from(aut).to(index));
