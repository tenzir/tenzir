...
template <class... Ts>
auto make_slice(table_slice_encoding encoding, record_type layout,
                const Ts&... xs) {
  auto builder = factory<table_slice_builder>::make(encoding, layout);
  if (!builder)
    FAIL("failed to create table slice builder");
  auto ok = builder->add(xs...);
  if (!ok)
    FAIL("builder failed to add given data");
  auto slice = builder->finish();
  if (slice.encoding() == table_slice_encoding::invalid)
    FAIL("builder failed to produce a table slice");
  return slice;
}

template <class VastType, class... Ts>
auto make_single_column_slice(table_slice_encoding encoding, const Ts&... xs) {
  record_type layout{record_field{"foo", VastType{}}};
  return make_slice(encoding, layout, xs...);
}
...
TEST(manual table slice building) {
  ...
  record_type layout{{"map", map_type{integer_type{}, integer_type{}}},
                     {"int", integer_type{}}};
  auto builder
    = factory<table_slice_builder>::make(table_slice_encoding::arrow, layout);
  auto map1 = map{{1_i, 10_i}, {2_i, 20_i}};
  auto map2 = map{{3_i, 30_i}};
  CHECK(builder->add(map1, caf::none));
  CHECK(builder->add(map2, caf::none));
  auto slice = builder->finish();
...
TEST(single column - equality) {
  auto slice1 = make_single_column_slice<count_type>(
    table_slice_encoding::arrow, 0_c, 1_c, caf::none, 3_c);
  auto slice2 = make_single_column_slice<count_type>(
    table_slice_encoding::arrow, 0_c, 1_c, caf::none, 3_c);
...
TEST(single column - list of integers) {
  ...
  auto slice
    = make_slice(table_slice_encoding::arrow, layout, list1, caf::none, list2);
  REQUIRE_EQUAL(slice.rows(), 3u);
...
TEST(single column - list of strings) {
  ...
  auto slice
    = make_slice(table_slice_encoding::arrow, layout, list1, list2, caf::none);
  REQUIRE_EQUAL(slice.rows(), 3u);
  CHECK_VARIANT_EQUAL(slice.at(0, 0), make_view(list1));
...
TEST(single column - list of list of integers) {
  ...
  auto slice
    = make_slice(table_slice_encoding::arrow, layout, caf::none, list1, list2);
  REQUIRE_EQUAL(slice.rows(), 3u);
  CHECK_VARIANT_EQUAL(slice.at(0, 0), caf::none);
...
TEST(single column - map) {
  ...
  auto slice
    = make_slice(table_slice_encoding::arrow, layout, map1, map2, caf::none);
  REQUIRE_EQUAL(slice.rows(), 3u);
  CHECK_VARIANT_EQUAL(slice.at(0, 0), make_view(map1));
  CHECK_VARIANT_EQUAL(slice.at(1, 0), make_view(map2));
  CHECK_VARIANT_EQUAL(slice.at(2, 0), caf::none);
...
TEST(single column - serialization) {
  ...
  auto slice1 = make_single_column_slice<count_type>(
    table_slice_encoding::arrow, 0_c, 1_c, 2_c, 3_c);
