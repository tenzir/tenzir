...
vast::filesystem_actor::behavior_type
dummy_filesystem(std::reference_wrapper<
  ...
  return {
    ...
    [](vast::atom::status, vast::status_verbosity, vast::duration) {
      return vast::record{};
    },
    [](vast::atom::move,
...
vast::store_builder_actor::behavior_type dummy_store(
  ...
  return {
    ...
    [](vast::atom::status, [[maybe_unused]] vast::status_verbosity verbosity,
       vast::duration) {
      return vast::record{
        {"foo", "bar"},
      };
...
TEST(No dense indexes serialization when create dense index in config is false) {
  ...
  auto sut = sys.spawn(vast::active_partition, schema_, partition_id,
                       vast::accountant_actor{}, filesystem, caf::settings{},
  ...
  vast::passive_partition_state passive_state;
  ...
  auto col2_idx
    = vast::unpack_value_index(*indexes->Get(1)->index(), container);
  REQUIRE(col2_idx);
...
TEST(delegate query to the store) {
  ...
  std::map<std::filesystem::path, std::vector<vast::chunk_ptr>>
    last_written_chunks;
  auto filesystem = sys.spawn(dummy_filesystem, std::ref(last_written_chunks));
  auto sut = sys.spawn(vast::active_partition, schema_, vast::uuid::random(),
                       vast::accountant_actor{}, filesystem, caf::settings{},
                       index_config_, store_plugin,
                       std::make_shared<vast::taxonomies>());
  REQUIRE(sut);
  run();
  auto& state = deref<vast::active_partition_actor::stateful_impl<
    vast::active_partition_state>>(sut)
                  .state;
