...
TEST(evaluation - type extractor - string + duration) {
  ...
  CHECK_EQUAL(zeek_conn_log_slice.at(id, 1, string_type{}),
              make_data_view("jM8ATYNKqZg"));
}
...
TEST(evaluation - field extractor - orig_h + proto) {
  ...
  CHECK_EQUAL(zeek_conn_log_slice.at(last, 1, string_type{}),
              make_data_view("WfzxgFx2lWb"));
}
