...
struct fixture : fixture_base {
  ...
  void spawn_catalog() {
    catalog = self->spawn(vast::catalog, accountant_actor{},
  ...
  void spawn_index() {
    ...
    index = self->spawn(vast::index, accountant_actor{}, fs, catalog, indexdir,
                        defaults::store_backend, 10000, duration{}, 5, 5, 1,
                        indexdir, vast::index_config{});
  }

  void spawn_importer() {
    importer = self->spawn(vast::importer, directory / "importer", index,
                           accountant_actor{});
  }

  void spawn_exporter(query_options opts) {
    exporter
      = self->spawn(vast::exporter, opts,
