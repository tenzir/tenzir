...
TEST(query pruning with index config) {
  ...
  auto accountant = vast::accountant_actor{};
  auto store = vast::store_actor{};
  ...
  auto partition
    = self->spawn(vast::active_partition, vast::type{}, id, accountant, fs,
  ...
  // catalog state.
  auto& state
    = deref<vast::catalog_actor::stateful_impl<vast::catalog_state>>(catalog)
        .state;
