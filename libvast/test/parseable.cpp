...
TEST(byte) {
  ...
  auto u16 = uint16_t{0};
  CHECK(b16be(f, l, u16));
  CHECK(u16 == detail::to_host_order(uint16_t{0x0102}));
  CHECK(f == l);
  f = str.begin();
  l = f + 4;
  auto u32 = uint32_t{0};
  CHECK(b32be(f, l, u32));
  CHECK(u32 == detail::to_host_order(uint32_t{0x01020304}));
  CHECK(f == l);
  f = str.begin();
  l = f + 8;
  auto u64 = uint64_t{0};
  CHECK(b64be(f, l, u64));
  CHECK(u64 == detail::to_host_order(uint64_t{0x0102030405060708}));
  CHECK(f == l);
  MESSAGE("little endian");
  f = str.begin();
  l = f + 2;
  CHECK(b16le(f, l, u16));
  CHECK(u16 == detail::to_host_order(uint16_t{0x0201}));
  CHECK(f == l);
  f = str.begin();
  l = f + 4;
  CHECK(b32le(f, l, u32));
  CHECK(u32 == detail::to_host_order(uint32_t{0x04030201}));
  CHECK(f == l);
  f = str.begin();
  l = f + 8;
  CHECK(b64le(f, l, u64));
  CHECK(u64 == detail::to_host_order(uint64_t{0x0807060504030201}));
  CHECK(f == l);
...
TEST(byte - type promotion regression) {
  ...
  CHECK_EQUAL(x, detail::to_host_order(uint16_t{0x8d}));
  CHECK(b16le("\x8d\x00"s, x));
  CHECK_EQUAL(x, detail::to_host_order(uint16_t{0x8d}));
  uint32_t y;
  CHECK(b32be("\x00\x00\x00\x8d"s, y));
  CHECK_EQUAL(y, detail::to_host_order(uint32_t{0x8d}));
  CHECK(b32le("\x8d\x00\x00\x00"s, y));
  CHECK_EQUAL(y, detail::to_host_order(uint32_t{0x8d}));
  uint64_t z;
  CHECK(b64be("\x00\x00\x00\x00\x00\x00\x00\x8d"s, z));
  CHECK_EQUAL(z, detail::to_host_order(uint64_t{0x8d}));
