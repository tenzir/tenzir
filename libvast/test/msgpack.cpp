...
TEST(fixarray) {
  ...
  CHECK_EQUAL(builder.add_proxy(std::move(proxy)), 11u);
  auto o = object{buf};
...
TEST(array16) {
  ...
  CHECK_EQUAL(builder.add_proxy(std::move(proxy)), 53u);
  auto o = object{buf};
...
TEST(fixmap) {
  ...
  CHECK_EQUAL(builder.add_proxy(std::move(proxy)), 18u);
...
TEST(ext8 via proxy) {
  ...
  auto result = builder.add_proxy(std::move(proxy), 42);
