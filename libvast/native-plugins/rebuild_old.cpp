...
namespace vast::plugins::rebuild_old {
...
caf::message rebuild_command(const invocation& inv, caf::actor_system& sys) {
  // Read options.
  const auto all = caf::get_or(inv.options, "vast.rebuild_old.all", false);
  const auto undersized
    = caf::get_or(inv.options, "vast.rebuild_old.undersized", false);
  const auto parallel
    = caf::get_or(inv.options, "vast.rebuild_old.parallel", size_t{1});
  if (parallel == 0)
  ...
  auto query = system::read_query(inv, "vast.rebuild_old.read",
...
VAST_REGISTER_PLUGIN(vast::plugins::rebuild_old::plugin)
