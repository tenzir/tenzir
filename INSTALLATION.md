...
to the ones in `/etc/rc.d` and `/usr/local/etc/rc.d`) that allows
...
1. Copy the rc script to `/usr/local/etc/rc.d`.
   ...
   **Note**: this happens automatically when executing the `install` target.
...
3. Start the service:
   ...
   You can verify that VAST is up and running with the status command:

        vast status

During the first start, the rc script checks whether a `vast` user and group
exist already. These are necessary so that [daemon(8)][daemon] can start VAST
