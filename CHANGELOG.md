...
- ⚠️ Installed schema definitions now reside in `<datadir>/vast/schema/types` as
  opposed to `<datadir>/vast/schema` to distinguish them from taxonomy
  definitions. When overriding an existing installation, you _may_ have to
  delete the old schema definitions by hand.
