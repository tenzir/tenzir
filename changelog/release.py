...
def parse_yaml_changes(yaml_content):
    ...
    for line in yaml_content.split('\n'):
        ...
        if in_changes_section and line and not line.startswith(' ') and not line.startswith('-'):
            break
...
def create_yaml_content(title, description, changes):
    ...
    # Escape title and description if they contain special characters
    def escape_yaml_string(s):
        if not s:
            return '""'
        # If string contains quotes, newlines, or special chars, quote it
        if '"' in s or '\n' in s or s.strip() != s:
            # Escape quotes and wrap in quotes
            escaped = s.replace('"', '\\"')
            return f'"{escaped}"'
        # Check if it needs quoting (starts with special chars, etc.)
...
def create_release_file(version, title, description):
    ...
    if not unused_changes:
        print("No unused changes found. All changes have already been included in releases.", file=sys.stderr)
        sys.exit(1)
...
Examples:
  ...
  %(prog)s v5.3.1 --title "Tenzir Node v5.3.1" --description ""
    ...
    parser.add_argument("version", 
                       help="Version identifier for the release (used as filename)")
    parser.add_argument("--title", required=True,
                       help="Title of the release")
