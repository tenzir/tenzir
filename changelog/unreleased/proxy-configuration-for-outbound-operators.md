...
HTTP/HTTPS proxy. Set `tenzir.http-proxy` in `tenzir.yaml` (or via the
`HTTPS_PROXY` / `HTTP_PROXY` environment variables) to a URL such as
`http://proxy.example.com:3128`; Basic-auth userinfo is supported.

`tenzir.no-proxy` accepts the same suffix-list syntax as the `NO_PROXY`
environment variable and lets specific destinations bypass the proxy.
Loopback addresses (`localhost`, `127.0.0.0/8`, `::1`) always bypass.
