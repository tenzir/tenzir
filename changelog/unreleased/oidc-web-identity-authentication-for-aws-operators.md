...
The `web_identity` option accepts three token sources: `token_file` (path to a token file), `token_endpoint` (HTTP endpoint that returns a token), or `token` (direct token value). For HTTP endpoints, you can extract tokens from JSON responses using `token_path`.
