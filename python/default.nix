...
let
  ...
  dependencyPackages = map addDistOutput [
    ...
    pythonSet.pytz
    pythonSet.six
    pythonSet.tenzir-core
    pythonSet.tenzir-operator
    pythonSet.tzdata
  ];
  #dependencyPackages = builtins.map(x: addDistOutput x) (lib.attrsets.attrValues pythonSet);
in
{
  tenzir-wheelhouse = callPackage (
    { runCommand }:
    runCommand "tenzir-wheelhouse" { } ''
      set -eu
      mkdir -p $out
      copy_wheels() {
        local src="$1"
