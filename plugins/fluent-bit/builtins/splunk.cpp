...
public:
  ...
  auto parse_operator(parser_interface& p) const -> operator_ptr override {
    ...
    std::optional<location> splunk_send_raw = {};
    std::optional<located<std::string>> event_key = {};
    ...
    std::optional<located<std::string>> event_sourcetype_key = {};
    std::optional<located<std::string>> event_index = {};
    std::optional<located<std::string>> event_index_key = {};
    ...
    parser.add("--splunk-send-raw", splunk_send_raw);
    parser.add("--event-key", event_key, "<string>");
    ...
    parser.add("--event-sourcetype-field", event_sourcetype_key, "<string>");
    parser.add("--event-index", event_index, "<string>");
    parser.add("--event-index-field", event_index_key, "<string>");
    parser.add("--tls", tls);
    ...
    try_assign("splunk_send_raw", splunk_send_raw);
    try_assign("event_key", event_key);
    try_assign("event_host", event_host);
    try_assign("event_source", event_source);
    try_assign("event_sourcetype", event_sourcetype);
    try_assign("event_sourcetype_key", event_sourcetype_key);
    try_assign("event_index", event_index);
    try_assign("event_index_key", event_index_key);
    try_assign("tls", tls);
  ...
  auto make(invocation inv, session ctx) const
    ...
    argument_parser2::operator_("splunk")
      ...
      .add("channel", channel)
      .add("splunk_send_raw", splunk_send_raw)
      .add("event_key", event_key)
      .add("event_host", event_host)
      .add("event_source", event_source)
      .add("event_sourcetype", event_sourcetype)
      .add("event_sourcetype_field", event_sourcetype_key)
      .add("event_index", event_index)
      .add("event_index_field", event_index_key)
      .add("tls", tls)
      .add("cafile", cafile)
