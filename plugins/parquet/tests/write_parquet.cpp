...
TEST("Nova Parquet writer rejects saving and restoring checkpoints") {
  auto const* plugin = plugins::find<OperatorPlugin>("write_parquet");
  REQUIRE(plugin);
    ...
    for (auto loading : {false, true}) {
      ...
      try {
        ...
        CHECK_EQUAL(error.message,
                    "write_parquet does not support checkpoints yet");
  ...
  FAIL("Nova Parquet writer is not registered");
}
