...
@test "load HTTP POST" {
  load 50380 'load_http "http://localhost:50380", form=true, data={foo: "42"}'
}

@test "load HTTP POST form" {
  load 50381 'load_http "http://localhost:50381", form=true, data={foo: "42", bar: "baz"}'
}
...
@test "incompatible options" {
  check ! tenzir 'load_http "", form=true, json=true'
}
