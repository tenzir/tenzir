...
@test "Additional write options" {
  ...
  check tenzir "from ${BATS_TENZIR_DATADIR}/inputs/zeek/conn.log.gz read zeek-tsv | write feather --compression-level 7 | read feather | summarize count(.)"
  ...
  check ! tenzir "from ${BATS_TENZIR_DATADIR}/inputs/zeek/conn.log.gz read zeek-tsv | batch 256 | write feather --compression-level -1 --compression-type wrongname | read feather"
}
