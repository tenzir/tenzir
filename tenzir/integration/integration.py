#!/usr/bin/env python
"""tenzir core / tenzir integration test script v1
...
def run_step(
    ...
    try:
        ...
        cmd = basecmd + step.command if step.prepend_tenzir else step.command
        ...
        schema.And(
            {
                ...
                schema.Optional("prepend_tenzir", default=True): bool,
            },
            schema.Use(to_step),
        )
    )
...
def main():
    ...
    parser.add_argument(
        "--app", default="./core", help="Path to the executable (tenzir/core)"
    )
    parser.add_argument(
        "-s",
        "--set",
    ...
    # Load test set.
    if not args.set:
        args.set = Path(__file__).absolute().parent / "tests.yaml"
