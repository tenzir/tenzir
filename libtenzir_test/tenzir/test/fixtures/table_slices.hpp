...
private:
  using triple = std::tuple<int64_t, std::string, double>;

  caf::binary_serializer make_sink();

  tenzir::table_slice make_slice();

  tenzir::data_view at(size_t row, size_t col) const;
  ...
  tenzir::table_slice_builder_ptr builder;

  std::vector<std::vector<tenzir::data>> test_data;
