...
`6f9c841980b2333028b1ac19e2a21e99d96cbd36` on Sep 2, 2022, there are a total of
13,067 commits, with 2,335 being merges (17,9%). We'll take a deeper look at the
merge commits.
...
#| code-fold: true
#| label: since-v1.0
v1_0_0_rc1_time <- data |> filter(tag == "v1.0.0-rc1") |> pull(time)
since_v1_0_0_rc1 <- data |> filter(time >= v1_0_0_rc1_time)
rc <- since_v1_0_0_rc1 |> drop_na() |> filter(grepl("rc", tag))
non_rc <- since_v1_0_0_rc1 |> drop_na() |> filter(!grepl("rc", tag))

since_v1_0_0_rc1 |>
  ggplot(aes(x = time, y = count, label = tag)) +
    geom_point(size = 1, alpha = 0.2) +
    geom_segment(data = non_rc,
    ...
    geom_text_repel(size = 2,
                    ...
                    segment.color = "grey",
                    box.padding = 0.7) +
    geom_point(data = rc, aes(x = time, y = count),
               color = "blue",
               size = 2) +
    geom_point(data = non_rc, aes(x = time, y = count),
               color = "red",
               size = 2) +
