...
no_segment |>
  ggplot(aes(x = num_events, y = num_slices, size = bytes_in_storage, color = store_type)) +
...
no_segment |>
  ggplot(aes(x = num_events, y = bytes_memory, color = store_type)) +
...
median_parquet <- medians |> filter(store_type == "parquet") |> pull(median)
median_feather <- medians |> filter(store_type == "feather") |> pull(median)
median_original <- medians |> filter(store_type == "original") |> pull(median)
...
zeek_conn <- unified |>
  filter(store_type %in% c("parquet", "feather") & schema == "zeek.conn") |>
  pull(bytes)

suricata_snmp <- unified |>
  filter(store_type %in% c("parquet", "feather") & schema == "suricata.snmp") |>
  pull(bytes)

suricata_ikev2 <- unified |>
  filter(store_type %in% c("parquet", "feather") & schema == "suricata.ikev2") |>
  pull(bytes)
