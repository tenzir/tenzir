...
Since steps (1–3) are the same for both stores, we ignore them in the following
...
VAST and [made the Zstd compression level configurable][pr-2623].
...
parameters. Let's take a look at the data.
...
Between 1 and 100M events, we almost see everything. If we keep the order of the
schema from the above plot, but exchange number of event with event size in
memory, we can observe an inverse relationship between size and frequency:
...
a linear function of the number of stores. (We are aware of this limitation and
are exploring workarounds, but this topic is out of scope for this post.) The
...
the problem of linear state, but it provides a lot more mileage.
