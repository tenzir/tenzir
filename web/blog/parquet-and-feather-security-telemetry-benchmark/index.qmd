...
Batches into Parquet and Feather format. To avoid distortotion of small events,
we also restrict to schemas with more than 100k events.
...
unified |>
  ...
  summarize(score = median(bytes / duration)) |>
  pivot_wider(names_from = store_class,
              values_from = score,
              id_cols = c(schema, zstd.level)) |>
  ggplot(aes(x = parquet, y = feather,
             shape = zstd.level, color = zstd.level)) +
    geom_point(size = 3) +
    scale_x_log10(labels = scales::label_number(units = "auto_si")) +
    scale_y_log10(labels = scales::label_number(units = "auto_si")) +
    labs(x = "Parquet", y = "Feather",
         shape = "Zstd Level",
         color = "Zstd Level")
```
