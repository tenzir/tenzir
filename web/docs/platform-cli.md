...
The following commands allow setting up alerts for specific nodes. After a node
has been disconnected for the configured amount of time, the alert triggers
by performing a POST request against the configured webhook URL.
...
#### `<webhook_body>`
The body to send along with the webhook.
Must be valid JSON.
The body may contain the string `$NODE_ID`, which will be replaced
by the node id that triggered the alert.
Defaults to `{"text": "Node {node_id} disconnected for more than {duration}s"}`,
where node id and duration are set dynamically from the CLI parameters.
