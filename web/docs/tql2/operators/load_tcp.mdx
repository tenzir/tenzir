...
```tql
load_tcp endpoint:string, [parallel=int, local_endpoint=field, tls=bool,
...
### `local_endpoint = field (optional)`

Write a record with the fields `ip` and `port` resembling the local endpoint of
the respective TCP connection into the specified field at the end of the nested
pipeline.
