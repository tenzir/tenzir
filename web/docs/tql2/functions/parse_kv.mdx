...
```tql
parse_kv(input:string, field_sep:string, value_sep: string
...
The `parse_kv` operator parses a string as  Key-Value pairs.
...
output = input.parse_kv(r"\s*,\s*", r"\s*:\s*")
```
```tql
{
