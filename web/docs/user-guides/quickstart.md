...
[expression](../language/expressions.md) `#schema == "suricata.alert"`
...
We're going to show the data in [JSON format](../formats/json.md), but you could
display it in [other formats](../formats.md) as well.
...
:::info Schemas
In Tenzir, a [schema](../data-model/schemas.md) is just a record
[type definition](../data-model/type-system.md). You would touch
...
[`taste`](../operators/transformations/taste.md) operator limits the
...
[`select`](../operators/transformations/select.md) operator helps
...
[extractors](../language/expressions.md#extractors) locating data.
...
language](../language/expressions.md). Tenzir comes with native types for IP
...
look at various [operators](../operators.md) Tenzir has to offer and start
writing pipelines!
