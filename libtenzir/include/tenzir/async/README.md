...
- Create a session with `CurlSession::make(ctx.io_executor())`, configure
  `session.easy()` directly while the session is idle, then start one semantic
  transfer with `start_upload()` or `start_download()`.
...
- For uploads, call `start_upload()`, `push()` chunks, and eventually call
  ...
  `CurlTransferStatus::local_abort`, and `Err(CurlError{...})`, and to inspect
  terminal transfer metadata such as the response code. If the local producer
  fails, call `abort()` so libcurl fails the transfer callback instead of
  hanging.
- For downloads, call `start_download()` and drain `next()`. It returns chunk
  ...
  local abort, or `CurlError`. The terminal event includes transfer metadata
  such as the response code. The `buffer_capacity` argument controls how many
