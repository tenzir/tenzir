...
/// and recursion works via `p.extended_with("nested name")`
class field_path {
public:
  field_path() = default;
  ...
  public:
    ...
    auto extended_with(std::string_view field_name) const -> view {
      if (field_name.empty()) {
        return view{path_, 0};
      }
    ...
    auto extended_with(size_t index) const -> view {
      constexpr static auto buff_size
        = std::numeric_limits<size_t>::max_digits10 + 2;
      char buff[buff_size];
