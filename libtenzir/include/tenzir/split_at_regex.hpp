...
inline auto
split_at_regex(std::string_view separator, bool include_separator = false) {
  ...
  return [exp = std::move(expr),
    ...
    for (auto&& chunk : input) {
      ...
      while (begin <= buffer.end()
        ...
        if (include_separator) {
          co_yield std::string_view{current, what[0].second};
        } else {
    ...
    if (! buffer.empty()) {
      ...
      while (begin <= buffer.end()
             and boost::regex_search(begin, buffer.cend(), what, expr)) {
        if (include_separator) {
          auto event = std::string_view{current, what[0].second};
          co_yield event;
        } else {
