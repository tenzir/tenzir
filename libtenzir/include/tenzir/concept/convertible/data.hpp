...
caf::error convert(const list& src, To& dst, const list_type& t) {
  ...
  for (const auto& element : src) {
    ...
    if (auto err = convert(element, v, t.value_type()); err.valid()) {
...
caf::error convert(const map& src, To& dst, const map_type& t) {
  ...
  for (const auto& x : src) {
    auto err = [&] {
      ...
      if (auto err = convert(data_key, key, t.key_type()); err.valid()) {
        return err;
      }
      typename To::mapped_type value{};
      if (auto err = convert(data_value, value, t.value_type()); err.valid()) {
        return err;
      }
...
caf::error convert(const record& src, To& dst, const map_type& t) {
  ...
  for (const auto& x : src) {
    auto err = [&] {
      const auto& [data_key, data_value] = x;
      typename To::key_type key{};
      if (auto err = convert(data_key, key, kt); err.valid()) {
        return err;
      }
      typename To::mapped_type value{};
      if (auto err = convert(data_value, value, vt); err.valid()) {
        return err;
      }
      return detail::insert_to_map(dst, std::move(key), std::move(value));
    }();
...
caf::error convert(const list& src, To& dst, const map_type& t) {
  ...
  for (const auto& element : src) {
    ...
    if (auto err = convert(key->second, key_dst, kt); err.valid()) {
      return caf::make_error(
        ec::convert_error,
    ...
    if (stripped_record_prefix.empty()) {
      if (auto err = convert(*element_rec, value_dst, *rvt); err.valid()) {
        return caf::make_error(ec::convert_error,
