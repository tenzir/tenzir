  ...
  unpack(const fbs::coder::SingletonCoder& from, singleton_coder& to) {
    if constexpr (std::is_same_v<Bitmap, bitmap>) {
      ...
      return caf::make_error(
        ec::logic_error, "invalid tenzir.fbs.coder.SingletonCoder bitmap type");
    }
  }
      ...
      if constexpr (std::is_same_v<Bitmap, bitmap>) {
        ...
        if (!from_concrete)
          return caf::make_error(ec::logic_error,
                                 "invalid tenzir.fbs.coder.VectorCoder bitmap "
                                 "type");
...
public:
  ...
  unpack(const fbs::coder::MultiLevelCoder& from, multi_level_coder& to) {
    ...
    to.base_ = tenzir::base{std::move(base_values)};
    ...
    for (const auto& from_coder : *from.coders()) {
      ...
      if (!from_concrete_coder)
        return caf::make_error(ec::logic_error,
                               "invalid tenzir.fbs.coder.MultiLevelCoder coder "
                               "type");
      auto to_coder = Coder{};
