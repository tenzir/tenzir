...
template <>
struct fmt::formatter<tenzir::tql::projection> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const tenzir::tql::projection& x, FormatContext& ctx) const {
...
template <>
struct fmt::formatter<tenzir::tql::extractor> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const tenzir::tql::extractor& x, FormatContext& ctx) const {
    using namespace tenzir;
...
template <>
struct fmt::formatter<tenzir::tql::call_expr> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const tenzir::tql::call_expr& x, FormatContext& ctx) const {
...
template <>
struct fmt::formatter<tenzir::tql::meta_extractor> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const tenzir::tql::meta_extractor& x, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(), "{}", std::invoke([&] {
                            using enum tenzir::tql::meta_extractor;
                            switch (x) {
                              case schema:
                                return "#schema";
                            }
...
template <>
struct fmt::formatter<tenzir::tql::binary_expr> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const tenzir::tql::binary_expr& x, FormatContext& ctx) const {
...
template <>
struct fmt::formatter<tenzir::tql::expression> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const tenzir::tql::expression& x, FormatContext& ctx) const {
    return x.match(
      [&](const std::string& y) {
        // TODO: Escaping.
        return fmt::format_to(ctx.out(), "\"{}\"", y);
      },
      [&](const tenzir::offset& y) {
...
template <>
struct fmt::formatter<tenzir::tql::binary_op> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const tenzir::tql::binary_op& x, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(), "{}", std::invoke([&] {
                            using enum tenzir::tql::binary_op;
                            switch (x) {
                              case equals:
                                return "==";
                              case not_equals:
