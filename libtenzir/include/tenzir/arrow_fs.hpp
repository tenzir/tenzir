...
private:
  struct Partition {
    ...
    /// Fired by `finish_sub` so the deadline-based rotation timer wakes
    /// when the partition closes (size-based rotation, finalize) instead
    /// of lingering until `timeout` elapses.
    folly::CancellationSource cancel_timeout;
  };
  ...
  /// Marks partition `sk` as rotating, drops its `key_to_sub` mapping,
  /// and enqueues `RotateRequested{sk}` for `process_task` to close the
  /// sub. Acquires the state lock; safe to call from any task. No-op if
  /// the partition is gone or already rotating.
  auto rotate(int64_t sk) -> Task<void>;
