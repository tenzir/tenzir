...
public:
  ...
  friend auto inspect(Inspector& f, hasher& x) {
    ...
    if constexpr (!Inspector::is_loading) {
      ...
      return f.object(x).on_load(cb).fields(f.field("value", k));
    }
  }
...
class simple_hasher
  : public hasher<simple_hasher<HashFunction>,
    ...
    tenzir::detail::equality_comparable<simple_hasher<HashFunction>> {
  using super
...
public:
  ...
  friend auto inspect(Inspector& f, simple_hasher& x) {
    return f.object(x)
      .pretty_name("simple_hasher")
      .fields(f.field("value", static_cast<super&>(x)),
              f.field("seeds", x.seeds_));
  }
...
class double_hasher
  : public hasher<double_hasher<HashFunction>,
                  typename HashFunction::result_type>,
    tenzir::detail::equality_comparable<double_hasher<HashFunction>> {
  using super
