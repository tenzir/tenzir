...
public:
  ...
  auto with_prepended(std::string_view literal) const
  -> secret;
  /// Creates a new secret with `literal` appended to it.
  auto with_appended(std::string_view literal) const
  -> secret;
...
template<typename FlatbufferType>
auto as_bytes( const secret_common<FlatbufferType>& s) -> std::span<const std::byte> {
