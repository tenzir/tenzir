...
public:
  ...
  /// `await_query_id`.
  /// @pre `state() == idle`
  void start(tenzir::query_context query_context, index_actor index);
...
template <>
struct formatter<enum tenzir::query_processor::state_name> {
  ...
  auto format(enum tenzir::query_processor::state_name value,
