...
public:
  ...
  auto add(ViewType view) -> caf::error {
    ...
    using tenzir_type = type_from_data_t<view_value_type>;
    return add<tenzir_type>(view);
  }
...
private:
  ...
  type value_type;
  concrete_series_builder<tenzir::list_type>* list_builder_ = nullptr;
};
...
public:
  ...
  auto add(ViewType view) -> caf::error {
    using view_value_type = decltype(materialize(std::declval<ViewType>()));
    using tenzir_type = type_from_data_t<view_value_type>;
    return builder_provider_.provide().add<tenzir_type>(view);
  }
