...
//
//     sudo bpftrace -e 'usdt:/opt/tenzir/bin/tenzir:tenzir:catalog_lookup { printf("%d candidates\n", arg1); }'
//
...
//      of `tenzir:catalog_lookup`, and the binary would need to be named `tenzir`.
//
//
// # Inner Workings
//
...
/// Defines a USDT trace point for provider 'tenzir' with given parameters.
...
///
///               perf probe:        `sdt_tenzir:foo` or `%foo`
///               bpftrace:          `usdt:/path/to/libtenzir.so:tenzir:foo`
///               bpftrace (<= 0.8): `usdt:/path/to/tenzir:foo`
///               bcc:               `USDT("/path/to/libtenzir.so")
...
#define TENZIR_TRACEPOINT(name, ...) \
  FOLLY_SDT(tenzir, name, __VA_ARGS__)
...
//   (tenzir, name, __VA_ARGS__)
//
// #define TENZIR_SDT_IS_ENABLED(name)
//   (FOLLY_SDT_SEMAPHORE(tenzir, name) > 0)
