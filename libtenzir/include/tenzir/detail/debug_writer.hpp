...
public:
  ...
  /// add `<field_name>: <type_name> `
  bool begin_field_as(string_view field_name, std::string_view type_name);
  ...
  template <class... Args>
  void add_fmt(fmt::format_string<Args...> fs, Args&&... args) {
    if (top() == type::element) {
      pop();
    }
    fmt::format_to(out(), std::move(fs), std::forward<Args>(args)...);
  }
...
auto as_debug_writer(T& x) -> debug_writer* {
  ...
  if constexpr (std::is_same_v<U, debug_writer>) {
    return &x;
  } else if constexpr (std::is_same_v<U, caf::serializer>) {
