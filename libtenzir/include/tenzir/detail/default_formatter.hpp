...
struct fmt::formatter<T> {
  ...
  constexpr auto parse(fmt::format_parse_context& ctx)
    ...
    // if (mode == normal && not can_stringify && not can_inspect) {
    //   throw fmt::format_error{
    //     "the `{}` format specifier requires `to_string` or `inspect`"};
    // }
    if (mode == normal && not can_stringify) {
      throw fmt::format_error{"the `{}` format specifier requires `to_string`"};
    }
    if (mode != normal && not can_inspect) {
      throw fmt::format_error{
