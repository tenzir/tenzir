...
//
// - Repository: https://github.com/mariusbancila/croncpp
// - Commit:     450600388a176310399ae77f4571b0620abeab91
// - Path:       include/cronpp.hpp
...
struct cron_standard_traits {
  ...
  static const inline std::vector<std::string> MONTHS
...
class cronexpr {
  ...
  template <typename Traits>
  friend bool
  detail::find_next(cronexpr const& cex, std::tm& date, size_t const dot);
...
inline std::tm to_tm(CRONCPP_STRING_VIEW time) {
  ...
  int minute = 0;
  int second = 0;
  sscanf(time.data(), "%d-%d-%d %d:%d:%d", &year, &month, &day, &hour, &minute,
         &second);
...
split(CRONCPP_STRING_VIEW text, char const delimiter) {
...
CRONCPP_CONSTEXPTR inline bool
contains(CRONCPP_STRING_VIEW text, char const ch) noexcept {
...
static std::pair<cron_int, cron_int>
make_range(CRONCPP_STRING_VIEW field, cron_int const minval,
...
static void set_cron_days_of_week(std::string value, std::bitset<7>& target) {
  ...
  set_cron_field(days_replaced, target, Traits::CRON_MIN_DAYS_OF_WEEK,
...
static void set_cron_days_of_month(std::string value, std::bitset<31>& target) {
  ...
  set_cron_field(value, target, Traits::CRON_MIN_DAYS_OF_MONTH,
...
static void set_cron_month(std::string value, std::bitset<12>& target) {
  ...
  set_cron_field(month_replaced, target, Traits::CRON_MIN_MONTHS,
...
inline void
reset_all_fields(std::tm& date, std::bitset<7> const& marked_fields) {
...
template <size_t N>
static size_t
find_next(std::bitset<N> const& target, std::tm& date,
          unsigned int const minimum, unsigned int const maximum,
          unsigned int const value, cron_field const field,
          cron_field const next_field, std::bitset<7> const& marked_fields) {
...
static bool find_next(cronexpr const& cex, std::tm& date, size_t const dot) {
  ...
  auto updated_second
    = find_next(cex.seconds, date, Traits::CRON_MIN_SECONDS,
                Traits::CRON_MAX_SECONDS, second, cron_field::second,
                cron_field::minute, empty_list);
  ...
  auto update_minute
    = find_next(cex.minutes, date, Traits::CRON_MIN_MINUTES,
                Traits::CRON_MAX_MINUTES, minute, cron_field::minute,
                cron_field::hour_of_day, marked_fields);
  ...
  auto updated_hour
    = find_next(cex.hours, date, Traits::CRON_MIN_HOURS, Traits::CRON_MAX_HOURS,
                hour, cron_field::hour_of_day, cron_field::day_of_week,
                marked_fields);
  ...
  auto updated_day_of_month
    = find_next_day<Traits>(date, cex.days_of_month, day_of_month,
  ...
  auto updated_month
    = find_next(cex.months, date, Traits::CRON_MIN_MONTHS,
                Traits::CRON_MAX_MONTHS, month, cron_field::month,
                cron_field::year, marked_fields);
...
static cronexpr make_cron(CRONCPP_STRING_VIEW expr) {
  ...
  detail::set_cron_field(fields[0], cex.seconds, Traits::CRON_MIN_SECONDS,
                         Traits::CRON_MAX_SECONDS);
  detail::set_cron_field(fields[1], cex.minutes, Traits::CRON_MIN_MINUTES,
                         Traits::CRON_MAX_MINUTES);
  detail::set_cron_field(fields[2], cex.hours, Traits::CRON_MIN_HOURS,
