...
auto node(node_actor::stateful_pointer<node_state> self,
          std::filesystem::path dir, bool disable_forked_pipelines)
  -> node_actor::behavior_type {
  self->state().self = self;
  self->state().dir = std::move(dir);
  self->state().disable_forked_pipelines = disable_forked_pipelines;
  ...
  return {
    ...
    [self](const caf::exit_msg& msg) {
      ...
      for (auto& p : ps) {
        p.deliver(caf::make_error(ec::no_error));
      }
      ps.clear();
    ...
    [self](atom::set, endpoint endpoint) {
      ...
      if (not self->state().disable_forked_pipelines) {
        for (int i = 0; i < 5; i++) {
    ...
    [self](atom::spawn, atom::shell) -> caf::result<pipeline_shell_actor> {
      if (self->state().disable_forked_pipelines) {
