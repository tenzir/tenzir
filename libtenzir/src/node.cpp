...
auto spawn_index(node_actor::stateful_pointer<node_state> self,
  ...
  auto index = [&] {
    ...
    return self->spawn<caf::detached>(
      ...
      get_or(settings, "tenzir.max-resident-partitions",
             defaults::max_in_mem_partitions),
      get_or(settings, "tenzir.max-taste-partitions",
             defaults::taste_partitions),
      get_or(settings, "tenzir.max-queries", defaults::num_query_supervisors),
