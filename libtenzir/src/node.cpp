...
auto node(node_actor::stateful_pointer<node_state> self,
  ...
  return {
    ...
    [self](
      atom::resolve, atom::authentication, std::string name,
      std::string public_key) -> caf::result<platform_authentication_result> {
      auto store
        = self->system().registry().get<secret_store_actor>("tenzir.platform");
      if (not store) {
      ...
      self
        ->mail(atom::resolve_v, atom::authentication_v, std::move(name),
               std::move(public_key))
        .request(store, caf::infinite)
        .then(
