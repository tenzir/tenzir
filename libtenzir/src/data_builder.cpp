...
auto data_builder::commit_to(series_builder& builder, bool mark_dead,
  ...
  root_.commit_to(builder_ref{builder}, *this, seed, path_, mark_dead);
}

auto data_builder::materialize(bool mark_dead, const tenzir::type* seed)
  -> tenzir::data {
  tenzir::data res;
  root_.commit_to(res, *this, seed, path_, mark_dead);
  return res;
}
...
auto data_builder::append_signature_to(signature_type& sig,
                                       const tenzir::type* seed) -> void {
  root_.append_to_signature(sig, *this, seed, path_);
}
...
auto data_builder::emit_mismatch_warning(const type& value_type,
                                         ...
                                         const field_path::view& path) -> void {
  emit_or_throw(diagnostic::warning("parsed field `{}` contains `{}`, but the "
                                    "schema expects `{}`",
                                    path, value_type.kind(), seed_type.kind()));
}
...
auto node_record::append_to_signature(signature_type& sig,
  ...
  for (const auto& [k, index] : lookup_) {
    ...
    const auto key_bytes = as_bytes(k);
    sig.insert(sig.end(), key_bytes.begin(), key_bytes.end());
    field.append_to_signature(sig, rb, nullptr, path.extended_with(k));
  }
...
auto node_record::commit_to(tenzir::record_ref r, class data_builder& rb,
                            const tenzir::record_type* seed,
                            field_path::view path, bool mark_dead) -> void {
  ...
  for (auto& [k, v] : data_) {
    ...
    if (seed) {
      ...
      if (it != field_map->end()) {
        v.commit_to(r.field(k), rb, &(it->second), path.extended_with(k),
                    mark_dead);
    ...
    v.commit_to(r.field(k), rb, nullptr, path.extended_with(k), mark_dead);
  }
  if (mark_dead) {
    prune();
    mark_this_dead();
...
auto node_record::commit_to(tenzir::record& r, class data_builder& rb,
  ...
  for (auto& [k, v] : data_) {
    ...
    if (seed) {
      ...
      // If the field is in the seed
      if (it != field_map->end()) {
        v.commit_to(entry_it->second, rb, &(it->second), path.extended_with(k),
                    mark_dead);
        // No more work has to be done
        continue;
      }
    ...
    v.commit_to(entry_it->second, rb, nullptr, path.extended_with(k),
                mark_dead);
  }
  if (mark_dead) {
    prune();
...
auto node_object::try_resolve_nonstructural_field_mismatch(
  ...
  const auto visitor2 = detail::overload{
    ...
    [&rb, seed, &path]<non_structured_data_type T, typename S>(const T&,
                                                               const S&) {
      rb.emit_mismatch_warning(type{data_to_type_t<T>{}}, *seed, path);
    },
    // generic fallback
    ...
    // anything -> string
    [this]<fmt::formattable T>(const T& value, const string_type&) {
...
auto node_object::append_to_signature(signature_type& sig,
  ...
  const auto visitor = detail::overload{
    [&sig, &rb, seed, this, &path](node_list& v) {
      ...
      if (seed and not ls) {
        rb.emit_mismatch_warning("list", *seed, path);
        null();
        return false;
      }
      if (v.affects_signature() or ls) {
        v.append_to_signature(sig, rb, ls, path.as_is());
      }
      return true;
    },
    [&sig, &rb, seed, this, &path](node_record& v) {
      ...
      if (seed and not rs) {
        rb.emit_mismatch_warning(type{record_type{}}, *seed, path);
        null();
        return false;
      }
      if (v.affects_signature() or rs) {
        v.append_to_signature(sig, rb, rs, path.as_is());
      }
      return true;
    },
    [&sig, &rb, seed, this, &path](caf::none_t&) {
      ...
      if (seed) {
        if (auto sr = try_as<tenzir::record_type>(seed)) {
          auto r = record();
          r->append_to_signature(sig, rb, sr, path.as_is());
        ...
        if (auto sl = try_as<tenzir::list_type>(seed)) {
          auto* l = list();
          l->append_to_signature(sig, rb, sl, path.as_is());
...
auto node_object::commit_to(tenzir::builder_ref builder, class data_builder& rb,
                            const tenzir::type* seed, field_path::view path,
                            bool mark_dead) -> void {
  ...
  const auto visitor = detail::overload{
    [&builder, &rb, seed, mark_dead, &path](node_list& v) {
      ...
      // call `builder.list()`
      auto l = builder.list();
      v.commit_to(std::move(l), rb, ls, path.as_is(), mark_dead);
    },
    [&builder, &rb, seed, mark_dead, &path](node_record& v) {
      ...
      v.commit_to(std::move(r), rb, rs, path.as_is(), mark_dead);
    },
...
auto node_object::commit_to(tenzir::data& r, class data_builder& rb,
  ...
  const auto visitor = detail::overload{
    [&r, &rb, seed, mark_dead, &path](node_list& v) {
      ...
      r = tenzir::list{};
      v.commit_to(as<tenzir::list>(r), rb, ls, path.as_is(), mark_dead);
    },
    [&r, &rb, seed, mark_dead, &path](node_record& v) {
      ...
      r = tenzir::record{};
      v.commit_to(as<tenzir::record>(r), rb, rs, path.as_is(), mark_dead);
    },
  ...
  if (type_index_ == seed_index and not is_structural(type_index_)) {
    ...
    for (auto [i, v] : detail::enumerate(alive_elements())) {
      ...
      if (v.current_index() == type_index_list) {
        ...
        v.append_to_signature(sig, rb, seed_type_ptr, path.extended_with(i));
      }
...
auto node_list::commit_to(builder_ref r, class data_builder& rb,
                          const tenzir::list_type* seed, field_path::view path,
                          bool mark_dead) -> void {
  auto field_seed = seed ? seed->value_type() : tenzir::type{};
  for (auto [i, v] : detail::enumerate(alive_elements())) {
    v.commit_to(r, rb, seed ? &field_seed : nullptr, path.extended_with(i),
                mark_dead);
  }
...
auto node_list::commit_to(tenzir::list& l, class data_builder& rb,
  ...
  for (auto [i, v] : detail::enumerate(alive_elements())) {
    ...
    v.commit_to(d, rb, seed ? &field_seed : nullptr, path.extended_with(i),
                mark_dead);
  }
  TENZIR_ASSERT_EXPENSIVE(
    not mark_dead
