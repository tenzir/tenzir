...
constexpr const char* json_type_names[]
  = {"element", "object", "member", "array",          "string",
     "number",  "bool",   "null",   "special_object", "special_key"};
...
bool debug_writer::begin_field_as(string_view field_name,
                                  std::string_view type_name) {
  if (not begin_field(field_name)) {
    return false;
  }
  add(type_name);
  add(' ');
...
bool debug_writer::begin_field(string_view name, span<const type_id_t> types,
  ...
  auto result = query_type_name(types[index]);
  auto type_name = std::string_view{result.data(), result.size()};
  if (type_name.empty()) {
    emplace_error(sec::runtime_error, "query_type_name failed");
    return false;
  }
  if (type_name.starts_with("tenzir::")) {
    type_name = type_name.substr(8);
  }
  return begin_field_as(name, type_name);
  // if (not begin_key_value_pair()) {
  //   return false;
  // }
  // CAF_ASSERT(top() == type::key);
  // add("\"@");
...
bool debug_writer::begin_key_value_pair() {
  ...
  switch (t) {
    case type::object:
    case type::special_object:
      push(type::member);
      push(type::element);
      push(t == type::object ? type::key : type::special_key);
      return true;
    default: {
      std::string str = "expected object, found ";
      str += json_type_name(t);
...
bool debug_writer::begin_associative_array(size_t count) {
  switch (top()) {
    default:
    ...
    case type::element:
      // TODO: Improve this.
      // We display an associative array of size 1 as `key value`.
      if (count == 1) {
        unsafe_morph(type::special_object);
        return true;
      }
...
bool debug_writer::end_associative_array() {
  auto t = top();
  if (t == type::special_object ? pop() : pop_if(type::object)) {
    if (t != type::special_object) {
      --indentation_level_;
      nl();
      add('}');
    }
