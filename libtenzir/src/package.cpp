  ...
  for (const auto& [key, value] : data) {
    ...
    if (key == "args") {
        ...
        for (const auto& [subkey, subvalue] : *args_record) {
          if (subkey == "positional") {
            if (auto assigned = assign_parameters(
                  "args.positional", subvalue, result.args.positional,
                  positional_source);
                ! assigned) {
          ...
          if (subkey == "named") {
            if (auto assigned = assign_parameters("args.named", subvalue,
                                                  result.args.named,
                                                  named_source);
                ! assigned) {
              return assigned.error();
      ...
      if (try_as<view<list>>(&value)) {
        if (auto assigned = assign_parameters("args", value,
    ...
    if (key == "options") {
      ...
      if (auto assigned = assign_parameters("options", value,
                                            result.args.named, named_source);
          ! assigned) {
        return assigned.error();
      }
      continue;
