...
public:
  ...
  {
    auto path = ast::field_path::try_from(x);
    TENZIR_ASSERT(path);
    captures.push_back(std::move(*path));
  }

  auto result(this auto self) -> std::vector<ast::field_path> {
    return std::forward_like<decltype(self)>(self.captures);
  }

private:
  std::vector<ast::field_path> captures;
};
...
auto eval(const ast::lambda_expr& lambda, const basic_series<list_type>& input,
  ...
  return trace_panic(lambda, [&] -> multi_series {
    ...
    for (const auto& capture : captures) {
      if (capture.path().size() == 1
          and capture.path().front().id.name == lambda.left.name) {
        continue;
      }
