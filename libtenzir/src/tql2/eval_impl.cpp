...
auto evaluator::eval(const ast::record& x) -> multi_series {
  ...
  return map_series(items, [&](std::vector<series> items) {
...
auto evaluator::eval(const ast::list& x) -> multi_series {
  ...
  return map_series(items, [&](std::vector<series> items) -> series {
...
auto evaluator::eval(const ast::index_expr& x) -> multi_series {
  ...
  // TODO: Refactor.
  TENZIR_ASSERT(value.length() == index.length());
  auto v_part = size_t{0};
  auto i_part = size_t{0};
  value.part(v_part);
  // Algorithm:
  // 1. Take first part
  // 2. Slice larger to make them equal length
  // 3. => Yield first series pair
  // 4. Smaller part is done. Go to next part.
  // 5.
