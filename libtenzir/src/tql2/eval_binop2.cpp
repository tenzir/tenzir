...
constexpr auto
result_if_both_null(tenzir::ast::binary_op op) -> std::optional<bool> {
  using enum tenzir::ast::binary_op;
  switch (op) {
    case eq:
    case geq:
...
auto make_null_array(std::ptrdiff_t length,
                     memory::element_state state = memory::element_state::null)
  -> array_<data> {
...
struct BinOpKernel<Op, L, R> {
  ...
  using result = decltype(inner(std::declval<L>(), std::declval<R>()))::value_type;
...
template <tenzir::ast::binary_op Op, data_type L, data_type R>
  requires(is_relational(Op) and not(integral_type2<L> and integral_type2<R>)
           and not std::same_as<L, null> and not std::same_as<R, null>
           and requires(view_type_for_t<L> l, view_type_for_t<R> r) {
             l <=> r;
           })
struct BinOpKernel<Op, L, R> {
  using result = bool;
...
concept has_bin_op_kernel = requires { typename BinOpKernel<Op, L, R>::result; };
...
auto eval_op(evaluator& self, tenzir::ast::binary_expr const& x,
  ...
  if (not unsupported_l.empty()
    ...
    tenzir::diagnostic::warning(
      "binary operator `{}` not implemented for `{}` and `{}`", x.op.inner,
      unsupported_l, unsupported_r)
      .primary(x)
