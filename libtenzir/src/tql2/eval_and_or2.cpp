...
auto eval_and_or(evaluator& self, tenzir::ast::binary_expr const& x,
  ...
  for (auto row = std::ptrdiff_t{0}; row < self.length(); ++row) {
    ...
    if (not warned_left_type and lv.valid()
      ...
      auto kind = match(lv, []<data_type T>(array_row_view_<T>) -> std::string_view {
        if constexpr (std::same_as<T, null>) { return "null"; }
        ...
        else if constexpr (std::same_as<T, ip>) { return "ip"; }
        else if constexpr (std::same_as<T, subnet>) { return "subnet"; }
        else if constexpr (std::same_as<T, time>) { return "time"; }
        else if constexpr (std::same_as<T, duration>) { return "duration"; }
        else if constexpr (std::same_as<T, list>) { return "list"; }
        else { return "record"; }
      });
  ...
  auto left_flat
    = std::static_pointer_cast<arrow::BooleanArray>(tenzir::finish(left_builder));
  ...
  for (auto row = std::ptrdiff_t{0}; row < self.length(); ++row) {
    ...
    auto append_from_right = [&]() {
      ...
      if (not right_bool and rv.valid()
          and not warned_right_type
          and right_active.is_active(global_i)) {
        warned_right_type = true;
        auto kind = match(rv, []<data_type T>(array_row_view_<T>) -> std::string_view {
          if constexpr (std::same_as<T, null>) { return "null"; }
          else if constexpr (std::same_as<T, bool>) { return "bool"; }
          else if constexpr (std::same_as<T, std::int64_t>) { return "int64"; }
          else if constexpr (std::same_as<T, std::uint64_t>) { return "uint64"; }
...
template auto eval_and_or<tenzir::ast::binary_op::and_>(
  evaluator&, tenzir::ast::binary_expr const&, tenzir::ActiveRows const&)
  -> array_<data>;
template auto eval_and_or<tenzir::ast::binary_op::or_>(
  evaluator&, tenzir::ast::binary_expr const&, tenzir::ActiveRows const&)
  -> array_<data>;
