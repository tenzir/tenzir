...
auto assign(const ast::meta& left, series right, const table_slice& input,
  ...
  switch (left.kind) {
    case ast::meta::name: {
      ...
      // TENZIR_ASSERT(values->length() > 0);
      // auto begin = int64_t{0};
      // auto begin_name = values->IsValid(0) ? values->GetView(0) : "";
      // for (auto i = int64_t{0}; i < values->length() + 1; ++i) {
      //   // TODO: out of bounds
      //   auto name = values->IsValid(i) ? values->GetView(i) : "";
      //   auto emit = name != begin_name or i == values->length();
      //   if (emit) {
      //     auto new_slice = subslice(input, begin, i);
      //     auto new_type = type{
      //       begin_name,
      //       new_slice.schema(),
      //       collect(new_slice.schema().attributes()),
      //     };
      //     auto new_batch = check(
      //       to_record_batch(input)->ReplaceSchema(new_type.to_arrow_schema()));
      //     result.emplace_back(new_batch, std::move(new_type));
      //     begin = i;
      //     begin_name = name;
      //   }
      // }
