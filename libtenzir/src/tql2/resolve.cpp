  ...
  void visit(ast::entity& x) {
    ...
    const auto report_not_found = [&](const std::vector<ast::identifier>& path,
      ...
      if (! module_entities.empty()) {
        ...
        auto best = std::ranges::max(
          module_entities, {}, [&](const auto& cand) {
            return detail::calculate_similarity(target, cand);
          });
        if (detail::calculate_similarity(target, best) > -5) {
          suggestion = prefix.empty() ? best
                                      : fmt::format("{}::{}", prefix, best);
        }
        builder = std::move(builder).note("available {} in `{}`: {}", type_plural,
