...
public:
  explicit format_context(const format_config& config) : config_{config} {
  }
...
auto should_break_after(token_kind kind, const format_context& /* ctx */)
  -> bool {
  using enum token_kind;
...
auto format_tokens(std::span<const token> tokens, std::string_view source,
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    if (tok.kind == token_kind::line_comment
        || tok.kind == token_kind::delim_comment) {
    ...
    if (needs_space_before
        || (needs_operator_spacing(tok.kind) && ! ctx.at_line_start()
            && ! prev_was_operator)
        || (tok.kind == token_kind::lbrace && ! ctx.at_line_start()
            && ! prev_added_space)
        || (tok.kind == token_kind::lbracket && ! ctx.at_line_start()
            && ! prev_added_space)
    ...
    if (needs_operator_spacing(tok.kind)
        || needs_keyword_spacing(tok.kind, content)) {
    ...
    if (i + 1 < tokens.size()) {
      ...
      next_forbids_space
        = (next_kind == token_kind::dot || next_kind == token_kind::lpar
