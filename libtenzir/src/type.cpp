...
type type::from_legacy_type(const legacy_type& other) noexcept {
  ...
  return match(
    ...
    [&](const legacy_string_type& string) noexcept {
      ...
      } else if (string.name() == "secret") {
        return type{secret_type{}, std::move(attributes)};
      }
...
enumeration_type::array_type::make(
  ...
  auto dict_builder = string_type::make_arrow_builder(arrow_memory_pool());
...
offset record_type::resolve_flat_index(size_t flat_index) const noexcept {
  ...
  TENZIR_ASSERT(false, "index out of bounds");
}
...
record_type::insert_before(std::vector<struct field> fields) noexcept {
  return
    [fields = std::move(fields)](
      const field_view& field) mutable noexcept -> std::vector<struct field> {
      ...
      transformations.push_back({
        *lindex,
        ([&, rfield = std::move(rfield)](
          ...
          if (auto result = match(std::tie(lfield.type, rfield.type),
                                  do_merge(lfield, rfield))) {
            return {{
