...
#include "tenzir/fbs/segment.hpp"
...
segment::lookup(const tenzir::ids& xs) const {
...
caf::expected<std::vector<table_slice>>
segment::erase(const tenzir::ids& xs) const {
...
tenzir::table_slice segment::get_slice_(size_t idx) const {
...
std::vector<const tenzir::fbs::FlatTableSlice*> segment::flat_slices_() const {
  auto result = std::vector<const tenzir::fbs::FlatTableSlice*>{};
  const auto* segment = flatbuffer_->segment_as_v0();
