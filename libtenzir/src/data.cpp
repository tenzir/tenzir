  ...
  auto f = detail::overload{
    ...
    [&](const secret& value) -> flatbuffers::Offset<fbs::Data> {
      const auto value_offset = fbs::data::CreateSecret(
        builder, builder.CreateString(value.unsafe_get_value_be_careful()),
        builder.CreateString(value.name()),
        static_cast<detail::secret_enum_underlying_type>(value.type()),
  ...
  switch (from.data_type()) {
    ...
    case fbs::data::Data::secret: {
      ...
      to.get_data().emplace<secret>(
        from_as_secret->value()->str(), from_as_secret->name()->str(),
        static_cast<secret_type_type>(from_as_secret->type()),
        static_cast<secret_encoding>(from_as_secret->type()));
      return caf::none;
    }
