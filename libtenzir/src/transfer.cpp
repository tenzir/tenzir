...
auto should_bypass_proxy(std::string_view target_url) -> bool {
  auto url = curl::url{};
  if (url.set(curl::url::part::url, target_url,
              curl::url::flags::default_scheme)
      != curl::url::code::ok) {
    return false;
  }
  auto [code, host] = url.get(curl::url::part::host);
  return code == curl::url::code::ok and host and bypass_proxy(*host);
}

/// Applies the process-wide proxy configuration to a libcurl handle.
/// `CURLOPT_PROXY` accepts the full URL (including userinfo for Basic
/// authentication). `CURLOPT_NOPROXY` gets Tenzir's effective bypass list,
/// including the implicit loopback entries.
auto apply_proxy_options(curl::easy& easy, std::string_view target_url)
  -> caf::error {
  auto const& ps = get_proxy_settings();
  if (ps.http_proxy) {
    auto proxy = should_bypass_proxy(target_url)
                   ? std::string_view{""}
                   : std::string_view{*ps.http_proxy};
    if (auto err = to_error(easy.set(CURLOPT_PROXY, proxy)); err.valid()) {
      return err;
    }
  }
...
auto transfer::reset() -> caf::error {
  ...
  if (tls.skip_peer_verification.inner) {
