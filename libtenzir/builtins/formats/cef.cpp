...
struct printer_args final {
  ast::expression extension{make_root_field("extension")};
  ast::expression cef_version{make_root_field("cef_version", "0")};
  ast::expression device_vendor{make_root_field("device_vendor", "Tenzir")};
  ast::expression device_product{
    make_root_field("device_product", "Tenzir Node")};
  ast::expression device_version{
    make_root_field("device_version", tenzir::version::version)};
  ast::expression signature_id{make_root_field("signature_id", "unspecified")};
  ast::expression name{make_root_field("name", "")};
  ast::expression severity{make_root_field("severity", "Unknown")};
  ...
  auto add_to(argument_parser2& p) -> void {
    ...
    p.named_optional("cef_version", cef_version, "string");
    p.named_optional("device_vendor", device_vendor, "string");
    p.named_optional("device_product", device_product, "string");
    p.named_optional("device_version", device_version, "string");
    p.named_optional("signature_id", signature_id, "string");
    p.named_optional("name", name, "string");
    p.named_optional("severity", severity, "string");
...
void append_extension(std::string& out, record_view3 attributes, location loc,
...
#undef CHECK_APPEND_VALUE
            if (auto* r = try_as<view3<record>>(*extension)) {
              append_extension(str, *r, args.extension.get_location(), ctx);
            } else {
