...
public:
  ...
  // ,
  // read_worker_handles_{std::move(other.read_worker_handles_)}
  {
  }
  ...
  auto start(OpCtx& ctx) -> Task<void> override {
    ...
    if (args_.jobs > 0) {
      ...
      // read_worker_handles_.reserve(args_.jobs);
      for (auto i = uint64_t{0}; i < args_.jobs; ++i) {
        // read_worker_handles_.push_back(
  ...
  auto process_task(Any result, Push<table_slice>& push, OpCtx& ctx)
    ...
    // if (slice) {
    total_outgoing_events += slice.rows();
    co_await push(std::move(slice));
    // }
  }
  ...
  auto process(chunk_ptr input, Push<table_slice>& push, OpCtx& ctx)
    ...
    ++total_incoming_events;
    if (not input or input->size() == 0) {
      co_return;
    }
    if (args_.jobs > 0) {
      co_await process_parallel(std::move(input));
      // co_await drain_parallel_output(push, 0);
    } else {
  ...
  auto finalize(Push<table_slice>& push, OpCtx& ctx) -> Task<void> override {
    TENZIR_INFO("read json finalie: total incoming chunks {}",
                total_incoming_events);
    if (args_.jobs > 0) {
      ...
      //   read_worker_handles_ | std::views::transform([](auto& handle) {
      //     return handle.join();
      //   }));
      co_return;
    }
...
private:
  ...
  // std::vector<AsyncHandle<void>> read_worker_handles_;
  // counters
  size_t total_incoming_events = 0;
  size_t total_outgoing_events = 0;
};
...
public:
  ...
  // write_worker_handles_{std::move(other.write_worker_handles_)}
  {
    opts_.tql = args_.tql;
  ...
  auto start(OpCtx& ctx) -> Task<void> override {
    ...
    // write_worker_handles_.reserve(args_.jobs);
    for (auto i = uint64_t{0}; i < args_.jobs; ++i) {
      // write_worker_handles_.push_back(
  ...
  auto finalize(Push<chunk_ptr>& push, OpCtx& ctx) -> Task<void> override {
    if (args_.jobs > 0) {
      ...
      co_await drain_parallel_output(push, args_.jobs);
      // co_await folly::coro::collectAllRange(
      //   write_worker_handles_ | std::views::transform([](auto& handle) {
      //     return handle.join();
      //   }));
      co_return;
    }
...
private:
  ...
  // std::vector<AsyncHandle<void>> write_worker_handles_;
};
