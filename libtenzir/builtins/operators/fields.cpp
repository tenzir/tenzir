...
class FieldsNova final : public Operator<void, nova::Events> {
public:
  explicit FieldsNova(FieldsArgs args) : args_{std::move(args)} {
  }

  auto start(OpCtx& ctx) -> Task<void> override {
    if (not node_is_in_process(ctx.actor_system())) {
      diagnostic::error(
        "`fields` is only available in node pipelines under `--nova`")
        .primary(args_.operator_location)
...
public:
  ...
  auto describe() const -> Description override {
    auto d = Describer<FieldsArgs, Fields, FieldsNova>{};
