...
public:
  auto make(invocation inv, session ctx) const
    ...
    auto pipe_expr = ast::expression{};
    TRY(argument_parser2::operator_(name())
          .positional("interval", interval)
          .named_optional("parallel", parallel)
          .positional("pipeline", pipe_expr, "{ … }")
          .parse(inv, ctx));
    ...
    diagnostic::warning(
      "using the `every` operator as a transformation is deprecated")
      .primary(inv.self)
      .hint("use `window create_timeout={} {{ … }}` instead", interval.inner)
      .emit(ctx);
    auto window_inv = operator_factory_plugin::invocation{};
    window_inv.self = inv.self;
    window_inv.args.emplace_back(ast::assignment{
      ast::field_path::from(located{"create_timeout", interval.source}),
      location::unknown,
      ast::constant{
        interval.inner,
        interval.source,
    ...
    return window_op->make(std::move(window_inv), ctx);
  }
};
