...
private:
  ...
  auto mark_done(cache_state state) -> void {
    ...
    for (auto& [offset, rp] : pending_positional_reads_) {
      if (offset == cached_events_.size() and rp.pending()) {
  ...
  auto read_at(uint64_t offset) -> caf::result<table_slice> {
    ...
    pending_positional_reads_[offset] = rp;
    return rp;
  }
  ...
  detail::flat_map<uint64_t, caf::typed_response_promise<table_slice>>
...
public:
  ...
  auto start(OpCtx& ctx) -> Task<void> override {
    ...
    auto result = co_await ctx
                    .mail(atom::create_v, args_.id, /*exclusive=*/true,
    ...
    TENZIR_WARN("WriteCacheSink: announcing to cache");
    auto announce_result
      = co_await ctx.mail(atom::announce_v, false).request(cache_);
  ...
  auto process(table_slice input, OpCtx& ctx) -> Task<void> override {
    ...
    TENZIR_WARN("WriteCacheSink: writing {} events", input.rows());
    auto result
      = co_await ctx.mail(atom::write_v, std::move(input)).request(cache_);
    if (not result) {
  ...
  auto finalize(OpCtx& ctx) -> Task<void> override {
    TENZIR_WARN("WriteCacheSink: finalize() sending write_done");
    auto result
      = co_await ctx.mail(atom::write_v, atom::done_v).request(cache_);
    TENZIR_WARN("WriteCacheSink: finalize() write_done returned");
    if (not result) {
      diagnostic::error(result.error())
...
public:
  ...
  auto start(OpCtx& ctx) -> Task<void> override {
    ...
    auto cache_manager = ctx.actor_system().registry().get<cache_manager_actor>(
      "tenzir.cache-manager");
    TENZIR_ASSERT(cache_manager);
    auto result = co_await ctx.mail(atom::get_v, args_.id, /*exclusive=*/false)
                    .request(cache_manager);
    if (not result) {
      diagnostic::error(result.error())
        .note("failed to retrieve cache")
...
public:
  ...
  auto finalize(Push<table_slice>& push, OpCtx& ctx) -> Task<void> override {
    ...
    while (true) {
      auto result
        = co_await ctx.mail(atom::read_v, read_offset_).request(cache_);
      if (not result) {
        diagnostic::error(result.error())
