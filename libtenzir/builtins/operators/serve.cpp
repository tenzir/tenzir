...
public:
  ...
  auto start(OpCtx& ctx) -> Task<void> override {
    ...
    auto result = co_await ctx.mail(atom::start_v, args_.id, args_.buffer_size)
  ...
  auto process(table_slice input, OpCtx& ctx) -> Task<void> override {
    ...
    auto result = co_await ctx.mail(atom::put_v, args_.id, std::move(input))
                    .request(serve_manager_);
    if (not result) {
      diagnostic::error(result.error())
  ...
  auto finalize(OpCtx& ctx) -> Task<void> override {
    auto result
      = co_await ctx.mail(atom::stop_v, args_.id).request(serve_manager_);
    if (not result) {
      diagnostic::error(result.error())
