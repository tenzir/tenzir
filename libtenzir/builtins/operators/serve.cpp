...
/serve-multi:
  post:
    ...
    description: "Returns events from a existing pipelines. The pipeline definitions must include a serve operator. By default, the endpoint performs long polling (`timeout: 5s`) and returns events as soon as they are available (`min_events: 1`)."
    requestBody:
...
template <typename T>
T round_up_to_multiple(T numToRound, T multiple) {
