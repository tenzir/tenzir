...
struct opensearch_args {
  ...
  located<uint64_t> port{9200, location::unknown};
  std::optional<located<std::string>> address;
  located<uint64_t> max_request_size{10 * 1024 * 1024, location::unknown};

  auto add_to(argument_parser2& p) {
    p.named_optional("port", port);
    p.named("address", address);
  ...
  auto validate(diagnostic_handler& dh) -> failure_or<void> {
    TENZIR_ASSERT(op);
    if (port.inner > std::numeric_limits<uint16_t>::max()) {
      diagnostic::error("invalid port").primary(port).emit(dh);
      return failure::promise();
    }
    if (address and address->inner.empty()) {
      diagnostic::error("`address` must not be empty")
        .primary(*address)
        .emit(dh);
    }
  ...
  auto operator()(operator_control_plane& ctrl) const
    ...
    auto server
      = http::with(ctrl.self().system())
          ...
          .accept(detail::narrow<uint16_t>(args_.port.inner),
                  args_.address ? args_.address->inner : "")
