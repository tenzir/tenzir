...
struct ChartArgs {
  ...
  located<std::string> position{"grouped", location::unknown};
  location y_loc = location::unknown;
...
template <chart_type Ty>
auto handle_y(ChartArgs& args, ast::expression& y, session ctx,
...
auto handle_xlimit(const ChartArgs& args, ast::binary_op op,
  ...
  return xlimit{std::move(limit), c.as_data(), std::move(expr)};
}
...
private:
  auto ensure_compiled(OpCtx& ctx) -> bool {
    ...
    if (auto result = handle_y<Ty>(args, raw_.y, s, name()); not result) {
      return false;
    }
    if (raw_.x_min) {
      auto result
  ...
  auto filter_input(table_slice slice, diagnostic_handler& dh)
    ...
    for (auto&& s : eval(expr, slice, dh)) {
      const auto* array = try_as<arrow::BooleanArray>(&*s.array);
      TENZIR_ASSERT(array);
      ...
      for (auto i = int64_t{1}; i < len + 1; ++i) {
        ...
        if (curr) {
          results.push_back(subslice(slice, begin, i));
        }
        curr = next;
        begin = i;
      }
...
auto describe_chart_area() -> tenzir::Description {
  auto d
...
auto describe_chart_bar() -> tenzir::Description {
  auto d
...
auto describe_chart_line() -> tenzir::Description {
  auto d
...
auto describe_chart_pie() -> tenzir::Description {
  auto d
