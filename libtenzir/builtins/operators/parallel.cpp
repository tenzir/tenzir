...
struct parallel_args {
  ...
  located<uint64_t> split_threshold{5'000, location::unknown};
  ...
  auto validate(session ctx) const -> failure_or<void> {
    ...
    if (split_threshold.inner == 0) {
      diagnostic::error("`split_threshold` must not be zero")
        .primary(split_threshold)
        .emit(ctx);
      return failure::promise();
    }
    return {};
  ...
  friend auto inspect(auto& f, parallel_args& x) -> bool {
    return f.object(x).fields(f.field("id", x.id), f.field("op", x.op),
                              ...
                              f.field("split_threshold", x.split_threshold));
  }
};
...
struct parallel_operator final : public operator_base {
  ...
  auto split_slices(generator<table_slice> input) const
    ...
    for (auto&& slice : input) {
      ...
      const auto splits = (rows / args_.split_threshold.inner) + 1;
      for (auto i = uint64_t{}; i < splits; ++i) {
        co_yield subslice(slice, args_.split_threshold.inner * i,
                          std::min(rows,
                                   args_.split_threshold.inner * (i + 1)));
      }
    }
  }
...
struct parallel final : public operator_plugin2<parallel_operator> {
  auto make(invocation inv, session ctx) const
    ...
    auto p = argument_parser2::operator_(name())
               .named_optional("jobs", args.jobs)
               .named_optional("split_threshold", args.split_threshold)
               .positional("{ … }", pipe);
    TRY(p.parse(inv, ctx));
    if (not pipe) {
      diagnostic::error("missing positional argument `{{ … }}`")
        .usage("parallel [jobs=int, split_threshold=int] { … }")
        .docs(p.docs())
