...
struct plugin : public virtual operator_plugin2<opensearch_operator> {
  ...
  auto save_properties() const -> save_properties_t override {
    return {.schemes = {name_}, .strip_scheme = true};
  }
...
TENZIR_REGISTER_PLUGIN(tenzir::plugins::opensearch::plugin{"opensearch"})
TENZIR_REGISTER_PLUGIN(tenzir::plugins::opensearch::plugin{"elasticsearch"})
