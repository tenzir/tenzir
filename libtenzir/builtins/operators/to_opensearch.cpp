...
struct opensearch_args {
  ...
  auto validate(diagnostic_handler& dh) -> failure_or<void> {
    if (not url.inner.ends_with("/_bulk")) {
      url.inner.append("/_bulk");
    }
...
public:
  ...
  auto create_metadata(
    std::string_view action, std::optional<std::optional<std::string_view>> idx,
    std::optional<std::optional<std::string_view>> id,
    const opensearch_args& args) -> std::optional<diagnostic_builder> {
    ...
    if (not valid_action) {
      return diagnostic::warning("unsupprted action `{}`", action)
        .primary(args.action)
        .note("skipping event")
  ...
  operator()(generator<table_slice> input,
    ...
    for (auto last = time::clock::now(); auto&& slice : input) {
      ...
      for (auto&& doc : docs.values()) {
        ...
        if (auto diag
            = b.create_metadata(**action, idx.next(), id.next(), args_)) {
          std::move(diag).value().primary(args_.operator_location).emit(dh);
          continue;
        }
...
class plugin : public virtual operator_plugin2<opensearch_operator> {
