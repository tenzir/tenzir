...
namespace tenzir::plugins::mode_histogram {
...
public:
  ...
  auto finish() -> data override {
    if constexpr (Kind == kind::mode) {
      ...
      // TODO: Do we want to sort by frequency?
      // std::ranges::sort(result, std::greater<>{}, [](const auto& x) {
      //   return caf::get<int64_t>(as_vector(caf::get<record>(x))[1].second);
      // });
      return result;
    }
  }
...
class plugin : public virtual aggregation_plugin {
  auto name() const -> std::string override {
    return Kind == kind::mode ? "mode" : "histogram";
  };
...
using histogram_plugin = plugin<kind::histogram>;

} // namespace

} // namespace tenzir::plugins::mode_histogram

TENZIR_REGISTER_PLUGIN(tenzir::plugins::mode_histogram::mode_plugin)
TENZIR_REGISTER_PLUGIN(tenzir::plugins::mode_histogram::histogram_plugin)
