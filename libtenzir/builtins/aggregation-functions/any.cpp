...
public:
  ...
  auto restore(chunk_ptr chunk) noexcept -> bool override {
    ...
    if (not fb) {
      TENZIR_WARN(
        "failed to restore `any` aggregation instance: invalid FlatBuffer");
      return false;
    }
    any_ = (*fb)->result();
    switch ((*fb)->state()) {
    ...
    TENZIR_WARN(
      "failed to restore `any` aggregation instance: unknown state value");
    return false;
  }
...
class plugin : public virtual aggregation_plugin,
  ...
  auto describe() const -> nova::AggregationDescription override {
    ...
    auto d = nova::AggregationDescriber<AllAnyArgs, AllAnyFunction<false>>{};
