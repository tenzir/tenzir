...
namespace tenzir::plugins::segment_store {
...
struct passive_store_state {
  ...
  using request
    = std::tuple<tenzir::query_context, caf::typed_response_promise<uint64_t>>;
  std::vector<request> deferred_requests = {};

  /// Holds erase requests that did arrive while the segment data
  /// was still being loaded from disk.
  using erasure
    = std::tuple<tenzir::ids, caf::typed_response_promise<uint64_t>>;
  ...
  caf::optional<tenzir::segment> segment = {};

  /// A readable name for this active store.
  std::string name = {};

private:
  caf::result<atom::done> erase(const tenzir::ids&);
};

// Handler for `tenzir::query` that is shared between active and passive stores.
// Returns a the number of events that match the query.
template <typename Actor>
caf::expected<uint64_t>
handle_lookup(Actor& self, const tenzir::query_context& query_context,
      ...
      [self, start](chunk_ptr chunk) {
        ...
        if (!seg) {
          TENZIR_ERROR("{} couldn't create segment from chunk: {}", *self,
                       seg.error());
          self->send_exit(self,
                          caf::make_error(tenzir::ec::format_error,
...
public:
  ...
  make_store_builder([[maybe_unused]] accountant_actor accountant,
                     ...
                     [[maybe_unused]] const tenzir::uuid& id) const override {
...
TENZIR_REGISTER_PLUGIN(tenzir::plugins::segment_store::plugin)
