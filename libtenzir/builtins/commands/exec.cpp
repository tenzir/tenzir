...
void print_node(std::span<tql::parse_tree::node> nodes, size_t index,
                size_t indent = 0) {
...
auto exec_impl(std::string content, std::unique_ptr<diagnostic_handler> diag,
  ...
  for (auto& token : tokens) {
    if (token.kind == tql::token_kind::error) {
      auto begin = size_t{0};
