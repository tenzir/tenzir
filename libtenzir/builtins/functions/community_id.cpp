  ...
  auto make_function(invocation inv, session ctx) const
    ...
    return function_use::make([args
      ...
      iter_series(
        ...
        [&](series src_ip_series, series dst_ip_series, series proto_series,
            series src_port_series, series dst_port_series,
            series seed_series) {
          ...
          for (auto i = int64_t{0}; i < eval.length(); ++i) {
            ...
            if (have_src_port and have_dst_port) {
              ...
              auto flow = make_flow(src_ip, dst_ip,
                                    detail::narrow_cast<uint16_t>(src_port),
                                    detail::narrow_cast<uint16_t>(dst_port),
                                    proto_type);
