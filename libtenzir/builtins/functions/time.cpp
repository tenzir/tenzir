...
class from_epoch_ns final : public method_plugin {
public:
  auto name() const -> std::string override {
  ...
  auto make_function(invocation inv,
    ...
    auto expr = ast::expression{};
    TRY(argument_parser2::function(name()).add(expr, "<int>").parse(inv, ctx));
    return function_use::make([expr = std::move(expr),
                               this](evaluator eval, session ctx) -> series {
      auto arg = eval(expr);
      auto f = detail::overload{
        ...
        [&](const arrow::Int64Array& arg) {
          ...
          for (auto i = 0; i < arg.length(); ++i) {
            ...
            check(append_builder(
              time_type{}, *b, time{duration{value_at(int64_type{}, arg, i)}}));
          }
          return series{duration_type{}, finish(*b)};
        },
          ...
          for (auto i = 0; i < arg.length(); ++i) {
            ...
            check(
              append_builder(time_type{}, *b,
                             time{duration{value_at(uint64_type{}, arg, i)}}));
          }
          return series{duration_type{}, finish(*b)};
        },
        [&](const arrow::DoubleArray& arg) {
          ...
          for (auto i = 0; i < arg.length(); ++i) {
            ...
            check(append_builder(
              time_type{}, *b,
              time{duration{int64_t(value_at(double_type{}, arg, i))}}));
          }
          return series{duration_type{}, finish(*b)};
        },
        [&](const auto&) {
...
TENZIR_REGISTER_PLUGIN(from_epoch_ns)
