...
struct mock_filesystem_state {
  std::optional<caf::typed_response_promise<tenzir::chunk_ptr>>
...
using mock_filesystem_actor
  = tenzir::typed_actor_fwd<auto(deliver_mmap_promise)->caf::result<void>>::
    extend_with<filesystem_actor>::unwrap;
...
mock_filesystem_actor::behavior_type mock_filesystem(
  ...
  auto mmap_rp
    = std::make_shared<caf::typed_response_promise<tenzir::chunk_ptr>>();
  return {
    [mmap_rp, self](deliver_mmap_promise) {
      REQUIRE(self->state.mmap_response_promise);
      self->state.mmap_response_promise->deliver(tenzir::chunk_ptr{});
    ...
    [](tenzir::atom::move, const std::filesystem::path&,
       const std::filesystem::path&) mutable -> caf::result<tenzir::atom::done> {
      FAIL("not implemented");
    },
    [](
      tenzir::atom::move,
      const std::vector<std::pair<std::filesystem::path, std::filesystem::path>>&)
      -> caf::result<tenzir::atom::done> {
      FAIL("not implemented");
    },
    ...
    [](tenzir::atom::erase, std::filesystem::path&) {
...
TEST(passive_partition - load) {
  ...
  auto aut
    = self->spawn(passive_partition, id, tenzir::accountant_actor{}, fs, path);
  sched.run();
  ...
  // response promise.
  self->send(aut, atom::status_v, status_verbosity::debug, duration{});
  sched.run();
  self->receive(
    [&](atom::done&) {
