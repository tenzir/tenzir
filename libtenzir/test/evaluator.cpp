...
tenzir::indexer_actor::behavior_type dummy_indexer(counts xs) {
  return {
...
struct fixture : fixtures::deterministic_actor_system_and_events {
  ...
  std::map<std::string, std::vector<indexer_actor>> indexers;

  void add_indexer(std::vector<indexer_actor>& container, counts data) {
  ...
  ids run_sut(expression expression, std::vector<evaluation_triple> triples,
              tenzir::ids ids_for_partition = {}) {
    auto eval = sys.spawn(evaluator, std::move(expression), std::move(triples),
  ...
  ids query_with_ids(std::string_view expr_str, tenzir::ids ids_for_partition) {
    auto expr = unbox(to<expression>(expr_str));
    std::vector<evaluation_triple> triples;
    auto resolved = resolve(expr, schema);
    TENZIR_ASSERT(resolved.size() > 0);
