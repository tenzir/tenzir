...
dummy_sink(stream_sink_actor<table_slice>::pointer self, size_t num_events,
           caf::actor overseer) {
  return {
...
struct importer_fixture : Base {
  importer_fixture(size_t table_slice_size)
    ...
    importer = this->self->spawn(tenzir::importer, dir, index_actor{},
                                 accountant_actor{});
  }
  ...
  auto make_source() {
    return tenzir::detail::spawn_container_source(
      this->self->system(), this->zeek_conn_log, importer);
  }

  auto make_zeek_source() {
    auto stream = unbox(
      tenzir::detail::make_input_stream(artifacts::logs::zeek::small_conn));
    auto reader = std::make_unique<format::zeek::reader>(caf::settings{},
                                                         std::move(stream));
    return this->self->spawn(source, std::move(reader), slice_size,
                             std::nullopt, tenzir::catalog_actor{},
                             tenzir::module{}, std::string{},
...
struct nondeterministic_fixture : nondeterministic_fixture_base {
  nondeterministic_fixture()
    : nondeterministic_fixture_base(
      tenzir::defaults::import::table_slice_size) {
    // nop
  }
