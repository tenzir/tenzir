...
#include "tenzir/concept/parseable/tenzir.hpp"
...
TEST(csv reader - simple) {
  ...
  CHECK(slices[1].at(0, 0)
        == data{unbox(to<tenzir::time>("2011-08-12T14:59:11.994970Z"))});
...
TEST(csv reader - reordered schema) {
  ...
  CHECK(slices[0].at(0, 9)
        == data{unbox(to<tenzir::time>("2011-08-12+14:59:11.994970"))});
  CHECK((slices[0].at(0, 10) == data{list{5555u, 0u}}));
  CHECK(slices[0].at(0, 11)
        == data{list{unbox(to<tenzir::time>("2019-04-30T11:46:13Z"))}});
