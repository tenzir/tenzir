...
struct fixture : public fixtures::deterministic_actor_system {
  ...
  using response_type = decltype(std::declval<caf::blocking_actor>().request(
    std::declval<actor_type>(), caf::infinite, tenzir::atom::status_v));
...
caf::behavior success_dummy() {
  return {[](tenzir::atom::status) {
    return tenzir::atom::ok_v;
  }};
}

caf::error test_error = caf::make_error(tenzir::ec::unspecified, "error");

caf::behavior error_dummy() {
  return {[](tenzir::atom::status) -> caf::result<tenzir::atom::ok> {
    return test_error;
  }};
}
...
namespace tenzir::plugins::compaction {
