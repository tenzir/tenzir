...
#include "tenzir/detail/spawn_generator_source.hpp"
...
query_cursor query(fixtures::deterministic_actor_system& fixture,
                   index_actor index, std::string_view expr) {
  auto& self = fixture.self;
  self->send(index, atom::evaluate_v,
             tenzir::query_context::make_extract("test", self,
...
size_t
receive_result(fixtures::deterministic_actor_system& fixture, index_actor index,
               const uuid& query_id, uint32_t hits, uint32_t scheduled) {
  auto& self = fixture.self;
  size_t result = 0;
...
struct fixture : fixtures::deterministic_actor_system_and_events {
  ...
  index_state& state() {
    return deref<caf::stateful_actor<index_state>>(index).state;
  }

  query_cursor query(std::string_view expr) {
