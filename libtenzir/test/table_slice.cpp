...
TEST(cast) {
  ...
  const auto expected_rows = std::vector<record>{
    record{
      {"foo", caf::none},
      {"ts", unbox(to<tenzir::time>("2009-11-18T08:00:21.486539"))},
      {"id",
       record{
         {"orig_p", uint64_t{68}},
    ...
    record{
      {"foo", caf::none},
      {"ts", unbox(to<tenzir::time>("2009-11-18T08:08:00.237253"))},
      {"id",
       record{
         {"orig_p", uint64_t{137}},
    ...
    record{
      {"foo", caf::none},
      {"ts", unbox(to<tenzir::time>("2009-11-18T08:08:13.816224"))},
      {"id",
       record{
         {"orig_p", uint64_t{137}},
         {"orig_h", unbox(to<ip>("192.168.1.102"))},
...
TEST(unflatten - invalid field names) {
  ...
  auto input = make_random_table_slices(1, 1, flat_schema)->front();
  auto output = unflatten(input, ".");
  REQUIRE_EQUAL(output.schema(), (type{flat_schema.name(),
                                       record_type{{"a",
                                                    record_type{
                                                      {"g", int64_type{}},
                                                      {"gh", int64_type{}},
