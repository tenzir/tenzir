...
TEST("const_eval materializes a scalar constant with its location") {
  auto dh = null_diagnostic_handler{};
  auto reg = registry{};
  auto expr = constant(std::int64_t{42});
  auto result = const_eval(expr, InstantiateCtx{dh, reg});
  REQUIRE(result);
  CHECK_EQUAL(result->inner, data{std::int64_t{42}});
  CHECK(result->source == expr.get_location());
}

TEST("const_eval materializes nested records and lists in order") {
  auto dh = null_diagnostic_handler{};
  auto reg = registry{};
  ...
  auto expected = record{
    {"a", std::int64_t{1}},
    {"b", std::string{"x"}},
    {"c", list{std::int64_t{1}, std::string{"a"}, caf::none}},
  };
  CHECK_EQUAL(result->inner, data{std::move(expected)});
}
...
TEST("const_eval evaluates unary and binary kernels over the scratch input") {
  ...
  REQUIRE(result);
  CHECK_EQUAL(result->inner, data{std::int64_t{-1}});
  ...
  REQUIRE(result);
  CHECK_EQUAL(result->inner, data{std::int64_t{3}});
}
...
TEST("try_const_eval yields the value of a constant expression") {
  ...
  REQUIRE(result);
  CHECK_EQUAL(result->inner, data{std::int64_t{7}});
  CHECK(std::move(dh).collect().empty());
}
...
TEST("materialize converts a nested row into legacy data") {
  ...
  CHECK_EQUAL(materialize(array.get(0)), data{std::move(expected)});
}
