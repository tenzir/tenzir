...
struct fixture {
  ...
  auto make_loader(std::string_view name, std::string args)
    ...
    loader_plugin = tenzir::plugins::find<tenzir::loader_parser_plugin>(name);
  ...
  const tenzir::loader_parser_plugin* loader_plugin;
...
TEST(file loader - unreadable file) {
  ...
  loader_plugin = tenzir::plugins::find<tenzir::loader_plugin>("file");
