...
TEST("fetch_authorization supports simple auth strategies") {
  auto basic = make_auth_entry("basic-supported", "basic");
  basic.emplace("username", "user");
  basic.emplace("password", "pass");
  auto api_key = make_auth_entry("api-key-supported", "api-key");
  api_key.emplace("api_key", "token");
  auto bearer = make_auth_entry("bearer-supported", "bearer-static");
  ...
  auto ctx = TestOpCtx{system};
  for (auto name :
       {"basic-supported", "api-key-supported", "bearer-supported"}) {
...
TEST("fetch_authorization authorizes simple strategies") {
  auto basic = make_auth_entry("basic-authorizes", "basic");
  basic.emplace("username", "user");
  basic.emplace("password", "pass");
  auto api_key = make_auth_entry("api-key-authorizes", "api-key");
  api_key.emplace("api_key", "token");
  auto bearer = make_auth_entry("bearer-authorizes", "bearer-static");
  bearer.emplace("token", "token");
  auto ts = TestSystem{
    {std::move(basic), std::move(api_key), std::move(bearer)},
  };
  ...
  auto ctx = TestOpCtx{system};
  auto basic_auth
    = folly::coro::blockingWait(fetch_authorization("basic-authorizes", ctx));
  REQUIRE(basic_auth);
  CHECK(has_header(basic_auth->headers, "Authorization", "Basic dXNlcjpwYXNz"));

  auto api_key_auth
    = folly::coro::blockingWait(fetch_authorization("api-key-authorizes", ctx));
  REQUIRE(api_key_auth);
  CHECK(has_header(api_key_auth->headers, "X-Api-Key", "token"));

  auto bearer_auth
    = folly::coro::blockingWait(fetch_authorization("bearer-authorizes", ctx));
  REQUIRE(bearer_auth);
  CHECK(has_header(bearer_auth->headers, "Authorization", "Bearer token"));
}

TEST("fetch_authorization caches simple strategies") {
  auto basic = make_auth_entry("basic-cache", "basic");
  basic.emplace("username", "user");
  basic.emplace("password", "pass");
  auto api_key = make_auth_entry("api-key-cache", "api-key");
  api_key.emplace("api_key", "token");
  auto bearer = make_auth_entry("bearer-cache", "bearer-static");
  bearer.emplace("token", "token");
  auto ts = TestSystem{
    {std::move(basic), std::move(api_key), std::move(bearer)},
  };
  auto& system = *ts.system;
  auto ctx = TestOpCtx{system};
  for (auto const name : {"basic-cache", "api-key-cache", "bearer-cache"}) {
...
TEST("fetch_authorization replaces cached entry on refresh") {
  auto original = make_auth_entry("bearer-refresh", "bearer-static");
  original.emplace("token", "old-token");
  auto first_ts = TestSystem{{std::move(original)}};
  auto first_ctx = TestOpCtx{*first_ts.system};
  auto first = folly::coro::blockingWait(
    fetch_authorization("bearer-refresh", first_ctx));
  REQUIRE(first);
  CHECK(has_header(first->headers, "Authorization", "Bearer old-token"));
  auto updated = make_auth_entry("bearer-refresh", "bearer-static");
  updated.emplace("token", "new-token");
  auto second_ts = TestSystem{{std::move(updated)}};
  auto second_ctx = TestOpCtx{*second_ts.system};
  auto refreshed = folly::coro::blockingWait(
    fetch_authorization("bearer-refresh", second_ctx, true));
  REQUIRE(refreshed);
  CHECK(has_header(refreshed->headers, "Authorization", "Bearer new-token"));
}
