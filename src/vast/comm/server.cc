...
void server::start(std::string const& addr, unsigned port, conn_handler handler)
{
  ...
  LOG(info, comm) << "server accepting connections on " << endpoint;
  start_accept();

  thread_ = std::move(std::thread([this] { io_service_.run(); }));
}
...
void server::handle_accept(connection_ptr new_connection,
