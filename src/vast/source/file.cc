  ...
  : file_(filename)
{
  ...
  if (file_)
    ...
    VAST_LOG_ERROR("file source @" << id() << " cannot read " << filename);
}
...
bool bro2::parse_header()
{
...

  {
    if (! next())
    {
      VAST_LOG_ERROR("could not extract second log line");
      return false;
    }
...

  {
    if (! next())
    {
      VAST_LOG_ERROR("could not extract third log line");
      return false;
    }
...

  {
    if (! next())
    {
      VAST_LOG_ERROR("could not extract fourth log line");
      return false;
    }
...

  {
    if (! next())
    {
      VAST_LOG_ERROR("could not extract fifth log line");
      return false;
    }
...

  {
    if (! next())
    {
      VAST_LOG_ERROR("could not extract sixth log line");
      return false;
    }
...

  {
    if (! next())
    {
      VAST_LOG_ERROR("could not extract seventh log line");
      return false;
    }
  ...
  if (file_.peek() == '#')
  {
    VAST_LOG_ERROR("more headers than VAST knows");
    return false;
  }
