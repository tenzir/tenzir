...
constexpr size_t cstr_size(const char (&)[Size]) {
  ...
  std::string parse_url(new_data_msg msg){
    ...
    std::string url = bufstr.substr(pos + 1, bufstr.find(' ', pos+1)-4);
    //aout(self) << "url:'" << url << "'" << endl;
    return url;
  }
  
  std::string create_response(std::string content) {
    std::string response("");
  ...
  behavior connection_worker(broker* self, connection_handle hdl) {
    ...
    return {
      [=](const new_data_msg& msg) {
        std::string url = parse_url(msg);
  
        std::string query = url.substr(url.find("query=")+6, url.length());
        ...
        std::string ans = create_response(content);
