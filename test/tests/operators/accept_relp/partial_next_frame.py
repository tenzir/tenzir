...
def main() -> None:
    ...
    try:
        ...
        with client:
            ...
            close = frame(3, "close")
            # Wait for the Syslog acknowledgement before completing the next
            # frame, as a low-window streaming client may do.
            client.sendall(frame(2, "syslog", b"complete") + close[:1])
            transaction, payload = recv_response(client)
            assert transaction == 2 and payload == b"200 OK"
            try:
                client.sendall(close[1:])
            except OSError:
                pass
        stdout, stderr = process.communicate(timeout=5)
        assert process.returncode == 0, stderr
        assert stdout.count('data: "complete"') == 1, stdout
        assert stdout.count("transaction_id: 2") == 1, stdout
        print("partial_next_frame_does_not_delay_ack: true")
    finally:
