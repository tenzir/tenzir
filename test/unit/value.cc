...
inline bool string_check(value const& value, std::string expected)
{
  std::stringstream ss;
  ss << value;
  if (ss.str() == expected)
...
BOOST_AUTO_TEST_CASE(size)
{
  ...
  std::cout
    << "A value occupies " << sizeof(value) << " bytes"
    << std::endl;
}
...
{
  value v1;
  BOOST_CHECK(v1.which() == invalid_type);
  BOOST_CHECK(string_check(v1, "<invalid>"));
  BOOST_CHECK_EQUAL(to_string(v1), "<invalid>");

  value v2(nil);
  BOOST_CHECK(string_check(v2, "<nil>"));
...
BOOST_AUTO_TEST_CASE(boolean)
{
  value v1(true);
  value v2(false);
  BOOST_CHECK(string_check(v1, "T"));
  BOOST_CHECK_EQUAL(to_string(v1), "T");
  BOOST_CHECK(string_check(v2, "F"));
  BOOST_CHECK_EQUAL(to_string(v2), "F");
  v1 = false;
  v2 = true;
  BOOST_CHECK(string_check(v1, "F"));
  BOOST_CHECK(string_check(v2, "T"));
  BOOST_CHECK(v1 != v2);
  BOOST_CHECK(v1 < v2);
...
BOOST_AUTO_TEST_CASE(integers)
{
  ...
  BOOST_CHECK(string_check(v1, "+42"));
  BOOST_CHECK_EQUAL(to_string(v1), "+42");
  BOOST_CHECK(string_check(v2, "42"));
  BOOST_CHECK_EQUAL(to_string(v2), "42");
  BOOST_CHECK(! (v1 != v2));  // Not comparable due to different signedness.
  v1 = -1;
  v2 = 0;
  BOOST_CHECK(string_check(v1, "-1"));
  BOOST_CHECK(string_check(v2, "+0"));
  v2 = -99999999;
  BOOST_CHECK(v1 > v2);
  BOOST_CHECK(v1 != v2);

  value v3(0.0);
  BOOST_CHECK(v3.which() == double_type);
  BOOST_CHECK(string_check(v3, "0.0000000000"));
  BOOST_CHECK(v3 == 0.0);
  v3 = 0.123456789;
  BOOST_CHECK(string_check(v3, "0.1234567890"));
  BOOST_CHECK_EQUAL(to_string(v3), "0.1234567890");
  BOOST_CHECK(v3 < 123.456789);
  BOOST_CHECK(v3.get<double>() == 0.123456789);
}
...
BOOST_AUTO_TEST_CASE(strings)
{
  ...
  BOOST_CHECK(string_check(v1, "c"));
  v1 = 'x';
  BOOST_CHECK(string_check(v1, "x"));

  value v2("foo");
  BOOST_CHECK_EQUAL(v2, "foo");
  BOOST_CHECK(string_check(v2, "foo"));
  ...
  BOOST_CHECK(string_check(v2, "quux"));
  ...
  BOOST_CHECK(v3.get<string>().is_heap_allocated());
  str.pop_back();
  v3 = str.data();   // ...and is placed back into the in-situe buffer.
  BOOST_CHECK(v3 == str.data());
  BOOST_CHECK(! v3.get<string>().is_heap_allocated());
...
BOOST_AUTO_TEST_CASE(containers)
{
  ...
  BOOST_CHECK(string_check(v1, "[foo, bar, baz]"));
  ...
  BOOST_CHECK(string_check(v3, "{-3 -> 300, -2 -> 200, -1 -> 150, +0 -> 42}"));
}
...
BOOST_AUTO_TEST_CASE(addresses)
{
  ...
  BOOST_CHECK(string_check(v1, "10.1.1.2"));
  v1 = address("127.0.0.1");
  BOOST_CHECK(v1.get<address>().is_loopback());
  BOOST_CHECK(string_check(v1, "127.0.0.1"));
...
BOOST_AUTO_TEST_CASE(prefixes)
{
  ...
  BOOST_CHECK(v1.which() == prefix_type);
  BOOST_CHECK(string_check(v1, "10.0.0.0/8"));
  v1 = prefix(address("127.0.0.1"), 32);
  BOOST_CHECK(string_check(v1, "127.0.0.1/32"));
  BOOST_CHECK(v1.get<prefix>().length() == 32);
}
...
BOOST_AUTO_TEST_CASE(ports)
{
  ...
  BOOST_CHECK(string_check(v1, "8/icmp"));
  v1 = port(25, port::tcp);
  BOOST_CHECK(string_check(v1, "25/tcp"));
