...
```tql
{
  ...
  process: {
    peak_memory_usage: int, // Peak memory usage during the runtime of the process.
    current_memory_usage: int, // Current memory usage of the entire process.
    swap_space_usage: int, // Swap space used by the process.
  },
  arrow: { // Information about memory allocated by Arrow buffers.
    bytes: {
      ...
      total: int, // Cummulative allocations during this run
    },
    allocations: {
      current: int, // Number of curret allocations
